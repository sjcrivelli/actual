...
test('Schedules: successfully complete schedules operations', async () => {
    ...
    const schedAccountId1 = await api.createAccount({ name: 'sched-test-account1', offbudget: true }, 1000);
    expect(await api.getIDByName('accounts', 'sched-test-account1')).toEqual(schedAccountId1);
    //check getIDByName works on payees
    const schedPayeeId1 = await api.createPayee({ name: 'sched-test-payee1' });
    expect(await api.getIDByName('payees', 'sched-test-payee1')).toEqual(schedPayeeId1);
