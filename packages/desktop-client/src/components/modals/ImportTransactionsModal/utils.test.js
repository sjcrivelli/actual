...
describe('Import transactions', function () {
    describe('date parsing', function () {
        ...
        it.each(invalidInputs)('should not parse string `$str` with order `$order`', function (_a) {
            var str = _a.str, order = _a.order;
            expect((0, utils_1.parseDate)(str, order)).toBe(null);
        });
        var validInputs = [
            {
                order: 'yyyy mm dd',
        ...
        describe.each(validInputs)('should parse with order `$order`', function (_a) {
            var order = _a.order, cases = _a.cases;
            it.each(cases)('given input %j expects output %j', function (input, output) {
                var parsed = (0, utils_1.parseDate)(input, order);
