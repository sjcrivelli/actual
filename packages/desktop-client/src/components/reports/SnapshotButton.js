...
exports.SnapshotButton = SnapshotButton;
...
function SnapshotButton(_a) {
    var selected = _a.selected, children = _a.children, style = _a.style, onSelect = _a.onSelect, title = _a.title;
    return (<tooltip_1.Tooltip placement="bottom start" content={<text_1.Text>{title}</text_1.Text>} style={__assign(__assign({}, styles_1.styles.tooltip), { lineHeight: 1.5, padding: '6px 10px' })}>
      <button_1.Button variant="bare" style={__assign(__assign({}, (selected && {
        ...
        })), style)} onPress={onSelect} aria-label={title}>
        {children}
      </button_1.Button>
    </tooltip_1.Tooltip>);
}
