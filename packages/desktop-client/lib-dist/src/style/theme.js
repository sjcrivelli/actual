...
export const themeOptions = Object.entries(themes).map(([key, { name }]) => [key, name]);
...
export function ThemeStyle() {
    ...
    const [themeColors, setThemeColors] = useState(undefined);
    useEffect(() => {
        if (activeTheme === 'auto') {
    ...
    const css = Object.entries(themeColors)
        ...
        .join('\n');
    return _jsx("style", { children: `:root {\n${css}}` });
}
