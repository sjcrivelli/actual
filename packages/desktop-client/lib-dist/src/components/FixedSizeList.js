...
export class FixedSizeList extends PureComponent {
    constructor(props) {
        ...
        this.getItemOffset = (index) => index * this.props.itemSize;
        this.getItemSize = () => this.props.itemSize;
        this.getEstimatedTotalSize = () => this.props.itemSize * this.props.itemCount;
        this.getOffsetForIndexAndAlignment = (index, align, scrollOffset) => {
            const size = this.props.height;
            const lastItemOffset = Math.max(0, this.props.itemCount * this.props.itemSize - size);
            const maxOffset = Math.min(lastItemOffset, index * this.props.itemSize);
            const minOffset = Math.max(0, index * this.props.itemSize - size + this.props.itemSize);
            if (align === 'smart') {
        ...
        this.getStartIndexForOffset = (offset) => Math.max(0, Math.min(this.props.itemCount - 1, Math.floor(offset / this.props.itemSize)));
        this.getStopIndexForStartIndex = (startIndex, scrollOffset) => {
            const offset = startIndex * this.props.itemSize;
            const size = this.props.height;
            const numVisibleItems = Math.ceil((size + scrollOffset - offset) / this.props.itemSize);
            return Math.max(0, Math.min(this.props.itemCount - 1, startIndex + numVisibleItems - 1));
        };
        ...
        this._resetIsScrollingDebounced = () => {
            ...
            this._resetIsScrollingTimeoutId = setTimeout(this._resetIsScrolling, IS_SCROLLING_DEBOUNCE_INTERVAL);
        };
    ...
    render() {
        const { className, height, header, innerRef, itemCount, renderRow, itemKey = defaultItemKey, useIsScrolling, width, } = this.props;
        ...
        return (_jsxs("div", { className: className, onScroll: this._onScrollVertical, ref: this._outerRefSetter, style: {
                height,
                width,
                overflow: 'hidden auto',
            }, children: [_jsx(View, { children: header }), _jsx("div", { ref: innerRef, style: {
                        position: 'relative',
    ...
    _getRangeToRender() {
        ...
        const { isScrolling, scrollDirection, scrollOffset: originalScrollOffset, } = this.state;
        const anchoredPos = this.getAnchoredScrollPos();
