...
describe('ReconcilingMessage math & UI', () => {
    ...
    test('shows "All reconciled!" when target matches cleared', async () => {
        ...
        render(_jsx(TestProvider, { children: _jsx(ReconcilingMessage, { balanceQuery: makeBalanceQuery(), targetBalance: 5000, onDone: onDone, onCreateTransaction: onCreateTransaction }) }));
        expect(screen.getByText('All reconciled!')).toBeInTheDocument();
        // No reconciliation transaction button when diff is zero
        expect(screen.queryByText('Create reconciliation transaction')).not.toBeInTheDocument();
    ...
    test('computes positive difference and passes correct amount', async () => {
        ...
        const onCreateTransaction = vi.fn();
        render(_jsx(TestProvider, { children: _jsx(ReconcilingMessage, { balanceQuery: makeBalanceQuery(), targetBalance: 10000, onDone: () => { }, onCreateTransaction: onCreateTransaction }) }));
...
describe('ReconcileMenu arithmetic evaluation', () => {
    ...
    const baseAccount = generateAccount('Checking', false, false);
    test('defaults input to cleared balance and submits evaluated integer amount', async () => {
        ...
        const onClose = vi.fn();
        render(_jsx(TestProvider, { children: _jsx(ReconcileMenu, { account: baseAccount, onReconcile: onReconcile, onClose: onClose }) }));
    ...
    test('clicking "Use last synced total" fills and submits that amount', async () => {
        ...
        const connectedAccount = generateAccount('Checking', true, false);
        connectedAccount.balance_current = 4321;
        render(_jsx(TestProvider, { children: _jsx(ReconcileMenu, { account: connectedAccount, onReconcile: onReconcile, onClose: onClose }) }));
