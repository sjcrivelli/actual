...
import { applyBudgetAction, createCategory, createCategoryGroup, deleteCategory, deleteCategoryGroup, getCategories, moveCategory, moveCategoryGroup, updateCategory, updateCategoryGroup, } from '@desktop-client/budget/budgetSlice';
...
function BudgetInner(props) {
    ...
    const [summaryCollapsed, setSummaryCollapsedPref] = useLocalPref('budget.summaryCollapsed');
    ...
    const onMonthSelect = async (month, numDisplayed) => {
        ...
        if (month < startMonth) {
            ...
            // pre-warm next month
            await prewarmMonth(budgetType, spreadsheet, monthUtils.addMonths(month, numDisplayed));
        }
    ...
    const categoryNameAlreadyExistsNotification = name => {
        dispatch(addNotification({
            notification: {
                type: 'error',
                message: t('Category “{{name}}” already exists in group (it may be hidden)', { name }),
            },
        }));
    };
    const onSaveCategory = async (category) => {
        const cats = await send('get-categories');
        const exists = cats.grouped
            .filter(g => g.id === category.group)[0]
            .categories.filter(c => c.name.toUpperCase() === category.name.toUpperCase())
            ...
            dispatch(pushModal({
                modal: {
                    ...
                    options: {
                        group: id,
                        onDelete: transferCategory => {
                            dispatch(deleteCategoryGroup({ id, transferId: transferCategory }));
                        },
                    },
                },
            }));
    ...
    const onReorderCategory = async (sortInfo) => {
        ...
        const exists = cats.grouped
            ...
            .categories.filter(c => c.name.toUpperCase() === moveCandidate.name.toUpperCase())
    ...
    const onReorderGroup = async (sortInfo) => {
        dispatch(moveCategoryGroup({ id: sortInfo.id, targetId: sortInfo.targetId }));
    };
    ...
    if (budgetType === 'tracking') {
        table = (_jsx(TrackingBudgetProvider, { summaryCollapsed: summaryCollapsed, onBudgetAction: onBudgetAction, onToggleSummaryCollapse: onToggleCollapse, children: _jsx(DynamicBudgetTable, { type: budgetType, prewarmStartMonth: startMonth, startMonth: startMonth, monthBounds: bounds, maxMonths: maxMonths, dataComponents: trackingComponents, onMonthSelect: onMonthSelect, onDeleteCategory: onDeleteCategory, onDeleteGroup: onDeleteGroup, onSaveCategory: onSaveCategory, onSaveGroup: onSaveGroup, onBudgetAction: onBudgetAction, onShowActivity: onShowActivity, onReorderCategory: onReorderCategory, onReorderGroup: onReorderGroup, onApplyBudgetTemplatesInGroup: onApplyBudgetTemplatesInGroup }) }));
    }
    else {
        table = (_jsx(EnvelopeBudgetProvider, { summaryCollapsed: summaryCollapsed, onBudgetAction: onBudgetAction, onToggleSummaryCollapse: onToggleCollapse, children: _jsx(DynamicBudgetTable, { type: budgetType, prewarmStartMonth: startMonth, startMonth: startMonth, monthBounds: bounds, maxMonths: maxMonths, dataComponents: envelopeComponents, onMonthSelect: onMonthSelect, onDeleteCategory: onDeleteCategory, onDeleteGroup: onDeleteGroup, onSaveCategory: onSaveCategory, onSaveGroup: onSaveGroup, onBudgetAction: onBudgetAction, onShowActivity: onShowActivity, onReorderCategory: onReorderCategory, onReorderGroup: onReorderGroup, onApplyBudgetTemplatesInGroup: onApplyBudgetTemplatesInGroup }) }));
    }
    return (_jsx(SheetNameProvider, { name: monthUtils.sheetForMonth(startMonth), children: _jsx(View, { style: { flex: 1 }, children: table }) }));
}
export function Budget() {
    const trackingComponents = useMemo(() => ({
    ...
    const envelopeComponents = useMemo(() => ({
    ...
    return (_jsx(View, { style: {
            ...
            paddingRight: 8,
            overflow: 'hidden',
        }, children: _jsx(BudgetInner, { trackingComponents: trackingComponents, envelopeComponents: envelopeComponents }) }));
}
