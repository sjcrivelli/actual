...
export function EnvelopeBudgetMenuModal({ categoryId, onUpdateBudget, onCopyLastMonthAverage, onSetMonthsAverage, onApplyBudgetTemplate, }) {
    ...
    const budgeted = useEnvelopeSheetValue(envelopeBudget.catBudgeted(categoryId));
    ...
    return (_jsx(Modal, { name: "envelope-budget-menu", children: ({ state: { close } }) => (_jsxs(_Fragment, { children: [_jsx(ModalHeader, { title: _jsx(ModalTitle, { title: category.name, shrinkOnOverflow: true }), rightContent: _jsx(ModalCloseButton, { onPress: close }) }), _jsxs(View, { style: {
