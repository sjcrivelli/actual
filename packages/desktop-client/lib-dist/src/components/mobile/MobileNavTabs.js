...
import { SvgAdd, SvgCog, SvgPiggyBank, SvgReports, SvgStoreFront, SvgTuning, SvgWallet, } from '@actual-app/components/icons/v1';
...
export function MobileNavTabs() {
    ...
    const openFull = useCallback(({ canceled }) => {
    ...
    const openDefault = useCallback((velocity = 0) => {
    ...
    ].map(tab => (_jsx(NavTab, { onClick: () => openDefault(), ...tab }, tab.path)));
    const bufferTabsCount = COLUMN_COUNT - (navTabs.length % COLUMN_COUNT);
    const bufferTabs = Array.from({ length: bufferTabsCount }).map((_, idx) => (_jsx("div", { style: navTabStyle }, idx)));
    useScrollListener(useCallback(({ isScrolling, hasScrolledToEnd }) => {
    ...
    const bind = useDrag(({ last, velocity: [, vy], direction: [, dy], offset: [, oy], cancel, canceled, }) => {
    ...
    return (_jsx(animated.div, { role: "navigation", ...bind(), style: {
            y,
            touchAction: 'pan-x',
            ...
            ...(!isNarrowWidth && { display: 'none' }),
        }, "data-navbar-state": navbarState, children: _jsxs(View, { children: [_jsx("div", { style: {
                        ...
                        height: TOTAL_HEIGHT,
                        width: '100%',
                    }, children: [navTabs, bufferTabs] })] }) }));
}
function NavTab({ Icon: TabIcon, name, path, style, onClick }) {
    return (_jsxs(NavLink, { to: path, style: ({ isActive }) => ({
            ...
            userSelect: 'none',
            ...style,
        }), onClick: onClick, children: [_jsx(TabIcon, { width: 22, height: 22, style: { minHeight: '22px' } }), name] }));
}
