...
function TransactionListWithPreviews() {
    ...
    const baseTransactionsQuery = useCallback(() => queries.transactions('onbudget').options({ splits: 'all' }).select('*'), []);
    ...
    const onBudgetAccountsFilter = useCallback((schedule) => onBudgetAccounts.some(a => a.id === schedule._account), [onBudgetAccounts]);
    ...
    const transactionsToDisplay = !isSearching
        ...
        : transactions;
    return (_jsx(TransactionListWithBalances, { isLoading: isSearching ? isTransactionsLoading : isPreviewTransactionsLoading, transactions: transactionsToDisplay, balance: balanceBindings.balance, isLoadingMore: isLoadingMore, onLoadMore: loadMoreTransactions, searchPlaceholder: t('Search On Budget Accounts'), onSearch: onSearch, onOpenTransaction: onOpenTransaction, showMakeTransfer: true }));
}
