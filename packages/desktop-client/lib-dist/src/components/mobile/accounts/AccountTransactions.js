...
export function AccountTransactions({ account, }) {
    const schedulesQuery = useMemo(() => getSchedulesQuery(account.id), [account.id]);
    return (_jsx(SchedulesProvider, { query: schedulesQuery, children: _jsx(TransactionListWithPreviews, { account: account }) }));
}
function TransactionListWithPreviews({ account, }) {
    const { t } = useTranslation();
    const baseTransactionsQuery = useCallback(() => queries.transactions(account.id).options({ splits: 'all' }).select('*'), [account.id]);
    const [transactionsQuery, setTransactionsQuery] = useState(baseTransactionsQuery());
    const { transactions, isLoading: isTransactionsLoading, reload: reloadTransactions, isLoadingMore, loadMore: loadMoreTransactions, } = useTransactions({
        query: transactionsQuery,
    });
    const { previewTransactions, isLoading: isPreviewTransactionsLoading } = useAccountPreviewTransactions({
    ...
    const transactionsToDisplay = !isSearching
        ...
        : transactions;
    return (_jsx(TransactionListWithBalances, { isLoading: isSearching ? isTransactionsLoading : isPreviewTransactionsLoading, transactions: transactionsToDisplay, balance: balanceBindings.balance, balanceCleared: balanceBindings.cleared, balanceUncleared: balanceBindings.uncleared, isLoadingMore: isLoadingMore, onLoadMore: loadMoreTransactions, searchPlaceholder: t('Search {{accountName}}', {
