...
export function OffBudgetAccountTransactions() {
    ...
    return (_jsx(SchedulesProvider, { query: schedulesQuery, children: _jsx(TransactionListWithPreviews, {}) }));
}
function TransactionListWithPreviews() {
    const { t } = useTranslation();
    const baseTransactionsQuery = useCallback(() => queries.transactions('offbudget').options({ splits: 'all' }).select('*'), []);
    ...
    const offBudgetAccountsFilter = useCallback((schedule) => offBudgetAccounts.some(a => a.id === schedule._account), [offBudgetAccounts]);
    ...
    const transactionsToDisplay = !isSearching
        ...
        : transactions;
    return (_jsx(TransactionListWithBalances, { isLoading: isSearching ? isTransactionsLoading : isPreviewTransactionsLoading, transactions: transactionsToDisplay, balance: balanceBindings.balance, isLoadingMore: isLoadingMore, onLoadMore: loadMoreTransactions, searchPlaceholder: t('Search Off Budget Accounts'), onSearch: onSearch, onOpenTransaction: onOpenTransaction, showMakeTransfer: true }));
}
