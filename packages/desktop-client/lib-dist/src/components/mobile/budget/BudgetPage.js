...
import { SvgArrowThinLeft, SvgArrowThinRight, SvgCheveronRight, } from '@actual-app/components/icons/v1';
import { SvgArrowButtonDown1, SvgCalendar, } from '@actual-app/components/icons/v2';
...
import { applyBudgetAction, createCategory, createCategoryGroup, deleteCategory, deleteCategoryGroup, updateCategory, updateCategoryGroup, } from '@desktop-client/budget/budgetSlice';
...
export function BudgetPage() {
    ...
    const onBudgetAction = useCallback(async (month, type, args) => {
    ...
    const onOpenNewCategoryModal = useCallback((groupId, isIncome) => {
        dispatch(pushModal({
            modal: {
                name: 'new-category',
                options: {
                    ...
                    onSubmit: async (name) => {
                        ...
                        dispatch(createCategory({ name, groupId, isIncome, isHidden: false }));
                    },
                },
            },
        }));
    }, [dispatch]);
    const onSaveGroup = useCallback(group => {
        dispatch(updateCategoryGroup({ group }));
    }, [dispatch]);
    const onApplyBudgetTemplatesInGroup = useCallback(async (categories) => {
        dispatch(applyBudgetAction({
            month: startMonth,
    ...
    const onDeleteGroup = useCallback(async (groupId) => {
    ...
    const onToggleGroupVisibility = useCallback(groupId => {
        const group = categoryGroups.find(g => g.id === groupId);
        onSaveGroup({
            ...group,
            hidden: !!!group.hidden,
    ...
    const onDeleteCategory = useCallback(async (categoryId) => {
    ...
    const onToggleCategoryVisibility = useCallback(categoryId => {
    ...
    const onOpenCategoryGroupNotesModal = useCallback(id => {
    ...
    const onOpenCategoryNotesModal = useCallback(id => {
    ...
    const onOpenCategoryGroupMenuModal = useCallback(id => {
        const group = categoryGroups.find(g => g.id === id);
        dispatch(pushModal({
            modal: {
    ...
    const onOpenCategoryMenuModal = useCallback(id => {
        const category = categories.find(c => c.id === id);
        dispatch(pushModal({
            modal: {
                name: 'category-menu',
    ...
    const onOpenBudgetMonthNotesModal = useCallback(month => {
        dispatch(pushModal({
            modal: {
                name: 'notes',
                options: {
    ...
    const onOpenBudgetMonthMenu = useCallback(month => {
        dispatch(pushModal({
            modal: {
    ...
    return (_jsx(Page, { padding: 0, header: _jsx(MobilePageHeader, { title: _jsx(MonthSelector, { month: startMonth, monthBounds: monthBounds, onOpenMonthMenu: onOpenBudgetMonthMenu, onPrevMonth: onPrevMonth, onNextMonth: onNextMonth }), leftContent: _jsxs(Button, { variant: "bare", style: { margin: 10 }, onPress: onOpenBudgetPageMenu, "aria-label": t('Budget page menu'), children: [_jsx(SvgLogo, { style: { color: theme.mobileHeaderText }, width: "20", height: "20" }), _jsx(SvgCheveronRight, { style: { flexShrink: 0, color: theme.mobileHeaderTextSubdued }, width: "14", height: "14" })] }), rightContent: !monthUtils.isCurrentMonth(startMonth) && (_jsx(Button, { variant: "bare", onPress: onCurrentMonth, "aria-label": t('Today'), style: { margin: 10 }, children: _jsx(SvgCalendar, { width: 20, height: 20 }) })) }), children: _jsx(SheetNameProvider, { name: monthUtils.sheetForMonth(startMonth), children: _jsx(SyncRefresh, { onSync: async () => {
                    dispatch(sync());
                }, children: ({ onRefresh }) => (_jsxs(_Fragment, { children: [_jsx(Banners, { month: startMonth, onBudgetAction: onBudgetAction }), _jsx(BudgetTable
                        // This key forces the whole table rerender when the number
                        // format changes
                        , { categoryGroups: categoryGroups, month: startMonth, onShowBudgetSummary: onShowBudgetSummary, onBudgetAction: onBudgetAction, onRefresh: onRefresh, onEditCategoryGroup: onOpenCategoryGroupMenuModal, onEditCategory: onOpenCategoryMenuModal }, `${numberFormat}${hideFraction}`)] })) }) }) }));
}
function Banners({ month, onBudgetAction }) {
    const { t } = useTranslation();
    const [budgetType = 'envelope'] = useSyncedPref('budgetType');
    return (_jsxs(GridList, { "aria-label": t('Banners'), style: { backgroundColor: theme.mobilePageBackground }, children: [_jsx(UncategorizedTransactionsBanner, {}), _jsx(OverspendingBanner, { month: month, onBudgetAction: onBudgetAction, budgetType: budgetType }), budgetType === 'envelope' && (_jsx(OverbudgetedBanner, { month: month, onBudgetAction: onBudgetAction }))] }));
}
...
function UncategorizedTransactionsBanner(props) {
    ...
    const transactionsQuery = useMemo(() => uncategorizedTransactions().select('*'), []);
    ...
    return (_jsx(GridListItem, { textValue: "Uncategorized transactions banner", ...props, children: _jsx(Banner, { type: "warning", children: _jsxs(View, { style: {
                    flex: 1,
                    flexDirection: 'row',
                    alignItems: 'center',
                    justifyContent: 'space-between',
                }, children: [_jsxs(Trans, { count: transactions.length, children: ["You have ", { count: transactions.length }, " uncategorized transactions (", { amount: format(totalUncategorizedAmount, 'financial') }, ")"] }), _jsx(Button, { onPress: () => navigate('/categories/uncategorized'), style: PILL_STYLE, children: _jsx(Text, { children: _jsx(Trans, { children: "Categorize" }) }) })] }) }) }));
}
function OverbudgetedBanner({ month, onBudgetAction, ...props }) {
    ...
    return (_jsx(GridListItem, { textValue: "Overbudgeted banner", ...props, children: _jsx(Banner, { type: "critical", children: _jsxs(View, { style: {
                    flexDirection: 'row',
                    alignItems: 'center',
                    justifyContent: 'space-between',
                            ...
                            gap: 10,
                                ...
                                alignItems: 'center',
                                gap: 10,
                            }, children: [_jsx(SvgArrowButtonDown1, { style: { width: 15, height: 15 } }), _jsx(Text, { children: _jsx(Trans, { children: "You have budgeted more than your available funds" }) })] }) }), _jsx(Button, { onPress: openCoverOverbudgetedModal, style: PILL_STYLE, children: _jsx(Trans, { children: "Cover" }) })] }) }) }));
}
function OverspendingBanner({ month, onBudgetAction, budgetType, ...props }) {
    ...
    const dispatch = useDispatch();
    const format = useFormat();
    const { categories: overspentCategories, totalAmount: totalOverspending } = useOverspentCategories({ month });
    const categoryGroupsToShow = useMemo(() => categoryGroups
    ...
    const { showUndoNotification } = useUndo();
    const onOpenCoverCategoryModal = useCallback(categoryId => {
    ...
    return (_jsx(GridListItem, { textValue: "Overspent banner", ...props, children: _jsx(Banner, { type: "critical", children: _jsxs(View, { style: {
                    flexDirection: 'row',
                    alignItems: 'center',
                    justifyContent: 'space-between',
                }, children: [_jsx(View, { style: {
                            ...
                            alignItems: 'center',
                            gap: 10,
                        }, children: _jsx(Text, { children: _jsxs(Trans, { count: numberOfOverspentCategories, children: ["You have ", { count: numberOfOverspentCategories }, " overspent categories (", { amount: format(totalOverspending, 'financial') }, ")"] }) }) }), _jsxs(Button, { onPress: onOpenCategorySelectionModal, style: PILL_STYLE, children: [budgetType === 'envelope' && _jsx(Trans, { children: "Cover" }), budgetType === 'tracking' && _jsx(Trans, { children: "View" })] })] }) }) }));
}
function MonthSelector({ month, monthBounds, onOpenMonthMenu, onPrevMonth, onNextMonth, }) {
    ...
    return (_jsxs(View, { style: {
            ...
            alignItems: 'center',
            flexDirection: 'row',
        }, children: [_jsx(Button, { "aria-label": t('Previous month'), variant: "bare", isDisabled: !prevEnabled, onPress: onPrevMonth, style: { ...arrowButtonStyle, opacity: prevEnabled ? 1 : 0.6 }, children: _jsx(SvgArrowThinLeft, { width: "15", height: "15" }) }), _jsx(Button, { variant: "bare", style: {
                    textAlign: 'center',
                    fontSize: 16,
                    fontWeight: 500,
                }, onPress: () => {
                    onOpenMonthMenu?.(month);
                }, "data-month": month, children: _jsx(Text, { style: styles.underlinedText, children: monthUtils.format(month, 'MMMM ‘yy', locale) }) }), _jsx(Button, { "aria-label": t('Next month'), variant: "bare", isDisabled: !nextEnabled, onPress: onNextMonth, style: { ...arrowButtonStyle, opacity: nextEnabled ? 1 : 0.6 }, children: _jsx(SvgArrowThinRight, { width: "15", height: "15" }) })] }));
}
