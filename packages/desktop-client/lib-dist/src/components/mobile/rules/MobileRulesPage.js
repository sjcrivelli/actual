...
export function MobileRulesPage() {
    ...
    const filteredRules = useMemo(() => {
        ...
        return filter === ''
            ? rules
            : rules.filter(rule => getNormalisedString(ruleToString(rule, filterData)).includes(getNormalisedString(filter)));
    ...
    const handleRuleDelete = useCallback(async (rule) => {
        try {
    ...
    return (_jsxs(Page, { header: _jsx(MobilePageHeader, { title: t('Rules'), rightContent: _jsx(AddRuleButton, {}) }), padding: 0, children: [_jsx(View, { style: {
                    flexDirection: 'row',
                    alignItems: 'center',
                ...
                }, children: _jsx(Search, { placeholder: t('Filter rules…'), value: filter, onChange: onSearchChange, width: "100%", height: styles.mobileMinHeight, style: {
                        backgroundColor: theme.tableBackground,
                        borderColor: theme.formInputBorder,
                    } }) }), _jsx(RulesList, { rules: filteredRules, isLoading: isLoading, onRulePress: handleRulePress, onRuleDelete: handleRuleDelete })] }));
}
