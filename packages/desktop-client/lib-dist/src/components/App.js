...
import { hasHiddenScrollbars, ThemeStyle, useTheme, } from '@desktop-client/style';
...
function AppInner() {
    ...
    useEffect(() => {
        ...
        async function init() {
            const serverSocket = await maybeUpdate(() => global.Actual.getServerSocket());
            dispatch(setAppState({
    ...
    return budgetId ? _jsx(FinancesApp, {}) : _jsx(ManagementApp, {});
}
function ErrorFallback({ error }) {
    return (_jsxs(_Fragment, { children: [_jsx(AppBackground, {}), _jsx(FatalError, { error: error })] }));
}
export function App() {
    const store = useStore();
    useEffect(() => handleGlobalEvents(store), [store]);
    const [hiddenScrollbars, setHiddenScrollbars] = useState(hasHiddenScrollbars());
    const dispatch = useDispatch();
    useEffect(() => {
    ...
    return (_jsxs(BrowserRouter, { children: [_jsx(ExposeNavigate, {}), _jsx(HotkeysProvider, { initiallyActiveScopes: ['app'], children: _jsx(SpreadsheetProvider, { children: _jsx(SidebarProvider, { children: _jsx(BudgetMonthCountProvider, { children: _jsx(DndProvider, { backend: HTML5Backend, children: _jsx(View, { "data-theme": theme, style: {
                                        height: '100%',
                                        display: 'flex',
                                        ...
                                        }, children: [_jsxs(ErrorBoundary, { FallbackComponent: ErrorFallback, children: [process.env.REACT_APP_REVIEW_ID &&
                                                        !Platform.isPlaywright && _jsx(DevelopmentTopBar, {}), _jsx(AppInner, {})] }), _jsx(ThemeStyle, {}), _jsx(ErrorBoundary, { FallbackComponent: FatalError, children: _jsx(Modals, {}) }), _jsx(UpdateNotification, {})] }, hiddenScrollbars ? 'hidden-scrollbars' : 'scrollbars') }) }) }) }) }) })] }));
}
