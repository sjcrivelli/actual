...
export function ManagePayeesWithData({ initialSelectedIds, }) {
    ...
    useEffect(() => {
        async function onUndo({ tables, messages, meta }) {
            ...
            await refetchOrphanedPayees();
            const targetId = meta && typeof meta === 'object' && 'targetId' in meta
                ? meta.targetId
                : null;
    ...
    function onViewRules(id) {
        dispatch(pushModal({ modal: { name: 'manage-rules', options: { payeeId: id } } }));
    }
    ...
    return (_jsx(ManagePayees, { payees: payees, ruleCounts: ruleCounts, orphanedPayees: orphans, initialSelectedIds: initialSelectedIds, onBatchChange: async (changes) => {
            ...
            const mergeIdsOrphans = mergeIds.filter(m => orphans.map(o => o.id).includes(m));
