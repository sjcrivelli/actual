...
export function SelectedTransactionsButton({ getTransaction, onShow, onDuplicate, onDelete, onEdit, onLinkSchedule, onUnlinkSchedule, onCreateRule, onRunRules, onSetTransfer, onScheduleAction, showMakeTransfer, onMakeAsSplitTransaction, onMakeAsNonSplitTransactions, onMergeTransactions, }) {
    ...
    const canMakeAsSplitTransaction = useMemo(() => {
        ...
        const areAllSameDateAndAccount = transactions.every(tx => tx &&
            tx.date === firstTransaction?.date &&
            tx.account === firstTransaction?.account);
        const areNoSplitTransactions = transactions.every(tx => tx && !tx.is_parent && !tx.is_child);
        const areNoReconciledTransactions = transactions.every(tx => tx && !tx.reconciled);
    ...
    const canUnsplitTransactions = useMemo(() => {
        ...
        const transactions = selectedIds.map(id => getTransaction(id));
        const areNoReconciledTransactions = transactions.every(tx => tx && !tx.reconciled);
        const areAllSplitTransactions = transactions.every(tx => tx && (tx.is_parent || tx.is_child));
    ...
    useHotkeys('s', () => !types.trans || linked ? onViewSchedule() : onLinkSchedule(selectedIds), {
        scopes: ['app'],
    }, [onLinkSchedule, onViewSchedule, linked, selectedIds]);
    // edit amount (only if we're not in a merge context)
    useHotkeys('m', () => !canMerge && onEdit('amount', selectedIds), hotKeyOptions, [onEdit, selectedIds]);
    // merge
    useHotkeys('m', () => canMerge && onMergeTransactions(selectedIds), hotKeyOptions, [onMergeTransactions, selectedIds]);
    return (_jsx(SelectedItemsButton, { id: "transactions", name: count => t('{{count}} transactions', { count }), 
            ...
            ...(!types.trans
                ...
                : [
                    ...
                    ...(canMerge
                        ...
                        : []),
                    Menu.line,
                    { type: Menu.label, name: t('Edit field'), text: '' },
