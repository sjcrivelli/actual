...
import { generateTransaction, generateAccount, generateCategoryGroups, } from 'loot-core/mocks';
import { initServer } from 'loot-core/platform/client/fetch';
import { addSplitTransaction, realizeTempTransactions, splitTransaction, updateTransaction, } from 'loot-core/shared/transactions';
...
function LiveTransactionTable(props) {
    ...
    return (_jsx(TestProvider, { children: _jsx(AuthProvider, { children: _jsx(SpreadsheetProvider, { children: _jsx(SchedulesProvider, { children: _jsx(SelectedProviderWithItems, { name: "transactions", items: transactions, fetchAllIds: () => Promise.resolve(transactions.map(t => t.id)), children: _jsx(SplitsExpandedProvider, { children: _jsx(TransactionTable, { ...props, transactions: transactions, loadMoreTransactions: () => { }, 
                                // @ts-ignore TODO:
...
const categories = categoryGroups.reduce((all, group) => (group.categories ? [...all, ...group.categories] : all), []);
...
function renderTransactions(extraProps) {
    ...
    const result = render(_jsx(LiveTransactionTable, { ...defaultProps, ...extraProps }));
    return {
        ...result,
        getTransactions: () => transactions,
        updateProps: (props) => render(_jsx(LiveTransactionTable, { ...defaultProps, ...extraProps, ...props }), { container: result.container }),
    };
}
function queryNewField(container, name, subSelector = '', idx = 0) {
    const field = container.querySelectorAll(`[data-testid="new-transaction"] [data-testid="${name}"]`)[idx];
    if (subSelector !== '') {
...
function queryField(container, name, subSelector = '', idx) {
    const field = container.querySelectorAll(`[data-testid="transaction-table"] [data-testid="${name}"]`)[idx];
    if (subSelector !== '') {
        return field.querySelector(subSelector);
    }
    return field;
...
expect.extend({
    payeesToHaveFavoriteStars(container, validPayeeListWithFavorite) {
        ...
        if (foundStarList.length !== validPayeeListWithFavorite.length ||
            ...
            return {
                message: () => `Expected ${validPayeeListWithFavorite.join(', ')} to have favorite stars.` +
            ...
            return {
                message: () => `Expected ${validPayeeListWithFavorite} to have favorite stars`,
                pass: true,
            };
        }
    },
...
describe('Transactions', () => {
    test('transactions table shows the correct data', () => {
        ...
        getTransactions().forEach((transaction, idx) => {
            ...
            expect(queryField(container, 'account', 'div', idx).textContent).toBe(accounts.find(acct => acct.id === transaction.account)?.name);
            expect(queryField(container, 'payee', 'div', idx).textContent).toBe(payees.find(p => p.id === transaction.payee)?.name);
            ...
            if (transaction.amount <= 0) {
                expect(queryField(container, 'debit', 'div', idx).textContent).toBe(integerToCurrency(-transaction.amount));
                expect(queryField(container, 'credit', 'div', idx).textContent).toBe('');
            }
            else {
                expect(queryField(container, 'debit', 'div', idx).textContent).toBe('');
                expect(queryField(container, 'credit', 'div', idx).textContent).toBe(integerToCurrency(transaction.amount));
            }
        });
    });
    ...
    test('dropdown automatically opens and can be filtered', async () => {
        ...
        ].length).toBe(categoryGroups.length + categories.length);
        await userEvent.clear(input);
        await userEvent.type(input, 'Gener');
    ...
    test('dropdown selects an item with keyboard', async () => {
        ...
        expect(getTransactions()[2].category).toBe(categories.find(category => category.name === 'Food')?.id);
        await userEvent.type(input, '[Enter]');
        await waitForAutocomplete();
        // The transactions data should be updated with the right category
        expect(getTransactions()[2].category).toBe(categories.find(category => category.name === 'General')?.id);
    ...
    test('dropdown selects an item when clicking', async () => {
        ...
        expect(getTransactions()[2].category).toBe(categories.find(c => c.name === 'Food')?.id);
        await userEvent.click(items[2]);
        await waitForAutocomplete();
        expect(getTransactions()[2].category).toBe(categories.find(c => c.name === 'General')?.id);
    ...
    test('dropdown hovers but doesn’t change value', async () => {
        ...
        // @ts-expect-error fix me
        expect(highlighted.textContent).not.toBe(categories.find(c => c.id === currentCategory)?.name);
    });
    test('dropdown payee displays on new transaction with account list column', async () => {
        ...
        expect(Array.from(renderedPayees.values()).map(p => p.getAttribute('data-testid'))).toStrictEqual([
            'Alice-payee-item',
            'Bob-payee-item',
    ...
    test.skip('dropdown invalid value resets correctly', async () => {
        ...
        const tooltipItems = container.querySelectorAll('[data-testid="category-item-group"]');
    ...
    test('adding a new transaction works', async () => {
        const { queryByTestId, container, getTransactions, updateProps } = renderTransactions();
    ...
    test('adding a new split transaction works', async () => {
        ...
        await waitForAutocomplete();
        await waitForAutocomplete();
        await userEvent.click(container.querySelector('[data-testid="add-split-button"]'));
    ...
    test('escape closes the new transaction rows', async () => {
        ...
        updateProps({ isAdding: true });
        const cancelButton = container.querySelectorAll('[data-testid="new-transaction"] [data-testid="cancel-button"]')[0];
    ...
    test('transaction can be selected', async () => {
        const { container } = renderTransactions();
        await editField(container, 'date', 2);
        const selectCell = queryField(container, 'select', '[data-testid=cell-button]', 2);
