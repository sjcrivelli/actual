...
function ErrorFallback() {
    return (_jsxs(_Fragment, { children: [_jsx("div", { children: _jsx("br", {}) }), _jsx(Text, { style: { ...styles.mediumText, color: theme.errorText }, children: _jsx(Trans, { children: "There was a problem loading your report" }) })] }));
}
...
export function GetCardData({ report, payees, accounts, categories, earliestTransaction, latestTransaction, firstDayOfWeekIdx, showTooltip, }) {
    ...
    const data = graphData && groupedData ? { ...graphData, groupedData } : graphData;
    return data?.data ? (_jsx(ErrorBoundary, { FallbackComponent: ErrorFallback, children: _jsx(ChooseGraph, { data: data, mode: report.mode, graphType: report.graphType, balanceType: report.balanceType, groupBy: report.groupBy, interval: report.interval, compact: true, style: { height: 'auto', flex: 1 }, intervalsCount: intervals.length, showTooltip: !isNarrowWidth && showTooltip }) })) : (_jsx(LoadingIndicator, {}));
}
