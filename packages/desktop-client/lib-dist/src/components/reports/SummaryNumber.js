...
export function SummaryNumber({ value, contentType, animate = false, suffix = '', loading = true, initialFontSize = 14, fontSizeChanged, }) {
    const { t } = useTranslation();
    const [fontSize, setFontSize] = useState(initialFontSize);
    const refDiv = useRef(null);
    const format = useFormat();
    let displayAmount = contentType === 'percentage'
    ...
    return (_jsxs(_Fragment, { children: [loading && _jsx(LoadingIndicator, {}), !loading && (_jsx(View, { ref: mergedRef, role: "text", "aria-label": value < 0
                ...
                }, children: _jsx("span", { "aria-hidden": "true", children: _jsx(PrivacyFilter, { children: displayAmount }) }) }))] }));
}
