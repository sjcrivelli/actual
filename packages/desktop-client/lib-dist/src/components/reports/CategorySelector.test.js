...
function makeCategory({ id, name, hidden = false, group = '', }) {
    return { id, name, hidden, group };
}
function makeCategoryGroup({ id, name, categories, }) {
...
describe('CategorySelector', () => {
    it('renders category group and category checkboxes', () => {
        render(_jsx(CategorySelector, { ...defaultProps }));
    ...
    it('calls setSelectedCategories when a category is selected', async () => {
        ...
        render(_jsx(CategorySelector, { ...defaultProps, setSelectedCategories: setSelectedCategories }));
    ...
    it('unselects all categories when Unselect All is clicked', async () => {
        const setSelectedCategories = vi.fn();
        render(_jsx(CategorySelector, { ...defaultProps, selectedCategories: [cat1, cat2, cat3, cat4], setSelectedCategories: setSelectedCategories }));
