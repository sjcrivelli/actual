...
export function makeQuery(name, startDate, endDate, interval, conditionsOpKey, filters) {
    const intervalGroup = interval === 'Monthly'
        ? { $month: '$date' }
        : interval === 'Yearly'
            ? { $year: '$date' }
            : { $day: '$date' };
    const intervalFilter = interval === 'Weekly'
        ? '$day'
