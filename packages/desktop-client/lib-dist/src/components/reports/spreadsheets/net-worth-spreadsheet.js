...
export function createSpreadsheet(start, end, accounts, conditions = [], conditionsOp = 'and', locale, interval = 'Monthly', firstDayOfWeekIdx = '0', format) {
    return async (spreadsheet, setData) => {
        ...
        const data = await Promise.all(accounts.map(async (acct) => {
            const [starting, balances] = await Promise.all([
                aqlQuery(q('transactions')
                    .filter({
                    [conditionsOpKey]: filters,
                    account: acct.id,
                    date: { $lt: startDate },
                })
                    .calculate({ $sum: '$amount' })).then(({ data }) => data),
                aqlQuery(q('transactions')
                    .filter({
                    [conditionsOpKey]: filters,
                })
...
function recalculate(data, startDate, endDate, locale, interval = 'Monthly', firstDayOfWeekIdx = '0', format) {
    ...
    const graphData = intervals.reduce((arr, intervalItem, idx) => {
        ...
        const tooltipFormat = interval === 'Daily'
            ? 'MMMM d, yyyy'
            : interval === 'Weekly'
                ? 'MMM d, yyyy'
                : interval === 'Yearly'
