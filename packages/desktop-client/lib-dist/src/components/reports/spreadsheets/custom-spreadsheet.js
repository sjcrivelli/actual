...
import { determineIntervalRange, trimIntervalDataToRange, trimIntervalsToRange, } from './trimIntervals';
import { categoryLists, groupBySelections, ReportOptions, } from '@desktop-client/components/reports/ReportOptions';
import { aqlQuery } from '@desktop-client/queries/aqlQuery';
export function createCustomSpreadsheet({ startDate, endDate, interval, categories, conditions = [], conditionsOp, showEmpty, showOffBudget, showHiddenCategories, showUncategorized, trimIntervals, groupBy = '', balanceTypeOp = 'totalDebts', sortByOp = 'desc', payees = [], accounts = [], graphType, firstDayOfWeekIdx, setDataCheck, }) {
    ...
    return async (spreadsheet, setData) => {
        ...
        const intervalData = intervals.reduce((arr, intervalItem, index) => {
            ...
            arr.push({
                date: d.format(d.parseISO(intervalItem), ReportOptions.intervalFormat.get(interval) || ''),
                ...stacked,
        ...
        const calcDataFiltered = calcData.filter(i => filterEmptyRows({ showEmpty, data: i, balanceTypeOp }));
        ...
        const sortedCalcDataFiltered = [...calcDataFiltered].sort(sortData({ balanceTypeOp, sortByOp }));
        const legend = calculateLegend(trimmedIntervalData, sortedCalcDataFiltered, groupBy, graphType, balanceTypeOp);
        setData({
