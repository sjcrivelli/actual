...
export function summarySpreadsheet(start, end, conditions = [], conditionsOp = 'and', summaryContent, locale) {
...
async function calculatePercentage(data, summaryContent, startDay, endDay) {
    ...
    try {
        const response = await send('make-filters-from-conditions', {
            conditions: summaryContent?.divisorConditions?.filter(cond => !cond.customName),
        });
        filters = response.filters;
    }
    catch (error) {
    ...
    const makeDivisorQuery = () => q('transactions')
        .filter({
        [conditionsOpKey]: filters,
    })
