...
export function createSpendingSpreadsheet({ conditions = [], conditionsOp, compare, compareTo, }) {
    ...
    const compareInterval = monthUtils.dayRangeInclusive(compare + '-01', endDate);
    return async (spreadsheet, setData) => {
        ...
        const [assetsTo, debtsTo] = await Promise.all([
            aqlQuery(makeQuery('assets', startDateTo, endDateTo, interval, conditionsOpKey, filters)).then(({ data }) => data),
            aqlQuery(makeQuery('debts', startDateTo, endDateTo, interval, conditionsOpKey, filters)).then(({ data }) => data),
        ]);
        ...
        const intervalData = days.map(day => {
            ...
            const dayData = months.map(month => {
                const data = intervals.reduce((arr, intervalItem) => {
                    const offsetDay = Number(intervalItem.substring(8, 10)) >= 28
                        ? '28'
            ...
            const indexedData = keyBy(dayData, 'month');
            return {
                months: indexedData,
                day,
