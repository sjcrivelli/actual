...
export function FloatableSidebar() {
    ...
    return isNarrowWidth ? null : (_jsx(View, { onMouseOver: sidebarShouldFloat
            ? e => {
                debouncedHideSidebar.cancel();
                e.stopPropagation();
                sidebar.setHidden(false);
            }
            : undefined, onMouseLeave: sidebarShouldFloat ? () => debouncedHideSidebar() : undefined, style: {
            ...
            transform: `translateY(${!sidebarShouldFloat ? -8 : 0}px)
                      translateX(${sidebarShouldFloat && sidebar.hidden ? '-100' : '0'}%)`,
