...
function select(row, selectExpressions) {
    return Object.fromEntries(selectExpressions.map(fieldName => [fieldName, row[fieldName]]));
}
    ...
    if (query.filterExpressions.length > 0) {
        ...
        if (filter.date != null) {
            ...
            return limitOffset(data
                ...
                .map(row => select(row, query.selectExpressions)), query.limit, query.offset);
        }
    }
    else if (query.offset != null || query.limit != null) {
        return limitOffset(data.map(row => select(row, query.selectExpressions)), query.limit, query.offset);
    }
...
function mockListen(name, listener) {
    ...
    return () => {
        ...
        eventListeners.set(name, arr.filter(l => l !== listener));
    };
}
...
function mockPagingServer(dataLength, { delay, eventType = 'select' } = {}) {
    const data = [];
    ...
    mockServer({
        send: async (name, args) => {
            switch (name) {
                case 'query':
                    const query = args;
                    tracer.event('server-query', eventType === 'select' ? query.selectExpressions : query);
                    if (delay) {
                        await wait(delay);
                    }
                    return {
...
describe('pagedQuery', () => {
    ...
    it('pagedQuery fetches up to a specific row', async () => {
        ...
        await tracer.expect('server-query', expect.objectContaining({
            selectExpressions: [{ result: { $count: '*' } }],
        }));
        await tracer.expect('server-query', expect.objectContaining({ filterExpressions: [{ id: 300 }] }));
        await tracer.expect('server-query', expect.objectContaining({
        ...
        await tracer.expect('data', data.slice(0, data.findIndex(row => row.date < item.date) + 20));
        await wait(1000);
    });
});
