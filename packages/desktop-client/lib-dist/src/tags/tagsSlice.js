...
export const reloadTags = createAppAsyncThunk(`${sliceName}/reloadTags`, async () => {
    const tags = await send('tags-get');
    return tags;
});
export const createTag = createAppAsyncThunk(`${sliceName}/createTag`, async ({ tag, color, description }) => {
    const id = await send('tags-create', { tag, color, description });
    return id;
});
export const deleteTag = createAppAsyncThunk(`${sliceName}/deleteTag`, async (tag) => {
    const id = await send('tags-delete', tag);
    return id;
});
export const deleteAllTags = createAppAsyncThunk(`${sliceName}/deleteAllTags`, async (ids) => {
    const id = await send('tags-delete-all', ids);
    return id;
});
export const updateTag = createAppAsyncThunk(`${sliceName}/updateTag`, async (tag) => {
    const id = await send('tags-update', tag);
    return id;
});
export const findTags = createAppAsyncThunk(`${sliceName}/findTags`, async () => {
    const tags = await send('tags-find');
    return tags;
});
