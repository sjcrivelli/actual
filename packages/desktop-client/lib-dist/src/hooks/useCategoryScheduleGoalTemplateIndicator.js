 ...
 */
export function useCategoryScheduleGoalTemplateIndicator({ category, month, }) {
    const { t } = useTranslation();
    const locale = useLocale();
    const { schedules, statuses: scheduleStatuses } = useCategoryScheduleGoalTemplates({
        category,
    });
    return useMemo(() => {
        const schedulesToDisplay = schedules
            ...
            .filter(schedule => monthUtils.monthFromDate(schedule.next_date) === month)
            .sort((a, b) => {
...
function getScheduleStatusDescription({ t, schedule, scheduleStatus, locale, }) {
    if (!schedule || !scheduleStatus) {
        return '';
    }
    const isToday = monthUtils.isCurrentDay(schedule.next_date);
    const distanceFromNow = monthUtils.formatDistance(schedule.next_date, monthUtils.currentDay(), locale, {
        addSuffix: true,
    });
