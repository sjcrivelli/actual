...
function makeSpreadsheet() {
    const cellObservers = {};
    const LRUValueCache = new LRUCache({ max: 1200 });
    const cellCache = {};
    let observersDisabled = false;
    class Spreadsheet {
        observeCell(name, callback) {
        ...
        bind(sheetName = GLOBAL_SHEET_NAME, binding, callback) {
...
export function SpreadsheetProvider({ children }) {
    ...
    return (_jsx(SpreadsheetContext.Provider, { value: spreadsheet, children: children }));
}
