...
import { amountToInteger, currencyToAmount, getNumberFormat, integerToAmount, integerToCurrency, parseNumberFormat, setNumberFormat, } from 'loot-core/shared/util';
    ...
    switch (type) {
        ...
        case 'financial': {
            ...
            return {
                ...
                formattedString: integerToCurrency(localValue, formatter, decimalPlaces),
            };
        }
        default:
...
export function useFormat() {
    ...
    const numberFormatConfig = useMemo(() => parseNumberFormat({
    ...
    const applyCurrencyStyling = useCallback((formattedNumericValue, currencySymbol) => {
        if (!currencySymbol) {
        ...
        const styledAmount = position === 'after'
    ...
    const formatDisplay = useCallback((value, type = 'string') => {
        const isFinancialType = type === 'financial' ||
        ...
        const { numericValue, formattedString } = format(value, type, intlFormatter, activeCurrency.decimalPlaces);
    ...
    const toAmount = useCallback((value) => integerToAmount(value, activeCurrency.decimalPlaces), [activeCurrency.decimalPlaces]);
    const fromAmount = useCallback((value) => amountToInteger(value, activeCurrency.decimalPlaces), [activeCurrency.decimalPlaces]);
    const forEdit = useCallback((value) => {
        const amount = toAmount(value);
        const decimalPlaces = hideFractionPref === 'true' ? 0 : activeCurrency.decimalPlaces;
    ...
    const fromEdit = useCallback((value, defaultValue = null) => {
        if (value == null) {
            return defaultValue;
        }
        ...
        let numericValue = evalArithmetic(trimmed, null);
