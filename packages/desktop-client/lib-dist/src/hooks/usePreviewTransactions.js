...
import { getUpcomingDays, extractScheduleConds, scheduleIsRecurring, getNextDate, getScheduledAmount, } from 'loot-core/shared/schedules';
...
export function usePreviewTransactions({ filter, options, } = {}) {
    ...
    const scheduleTransactions = useMemo(() => {
        ...
        const upcomingPeriodEnd = d.startOfDay(parseDate(addDays(today, getUpcomingDays(upcomingLength))));
        return schedulesForPreview
            ...
            if (isRecurring) {
                while (day <= upcomingPeriodEnd) {
                    ...
                    if (d.startOfDay(parseDate(nextDate)) > upcomingPeriodEnd)
                        break;
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        Promise.all(scheduleTransactions.map(transaction => 
        ...
        send('rules-run', { transaction })))
            ...
            if (!isUnmounted) {
                ...
                if (optionsRef.current?.calculateRunningBalances) {
                    ...
                    calculateRunningBalancesBottomUp(ungroupedTransactions, 
                    // Preview transactions are behaves like 'all' splits
                    'all', optionsRef.current?.startingBalance));
                }
                setIsLoading(false);
            }
        })
