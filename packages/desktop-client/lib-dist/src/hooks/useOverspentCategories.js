...
export function useOverspentCategories({ month, }) {
    const spreadsheet = useSpreadsheet();
    const [budgetType = 'envelope'] = useSyncedPref('budgetType');
    const { list: categories, grouped: categoryGroups } = useCategories();
    const categoryGroupsById = useMemo(() => groupById(categoryGroups), [categoryGroups]);
    const categoryBalanceBindings = useMemo(() => categories.map(category => [
        category.id,
        budgetType === 'tracking'
            ? trackingBudget.catBalance(category.id)
            : envelopeBudget.catBalance(category.id),
    ]), [budgetType, categories]);
    const categoryCarryoverBindings = useMemo(() => categories.map(category => [
        category.id,
        budgetType === 'tracking'
    ...
    return useMemo(() => {
        ...
        const categoriesToReturn = categories
            .filter(category => categoryIdsToReturn.includes(category.id) && !category.is_income)
            .filter(category => budgetType === 'tracking'
            ? !category.hidden && !categoryGroupsById[category.group]?.hidden
            : true);
        const amountsByCategory = new Map(categoriesToReturn.map(category => [
            category.id,
            overspendingByCategory[category.id],
        ]));
        const totalAmount = Array.from(amountsByCategory.values()).reduce((sum, value) => sum + value, 0);
        return {
