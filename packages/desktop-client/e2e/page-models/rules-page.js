export class RulesPage {
    ...
    async _fillRuleFields(data) {
        ...
        if (data.conditions) {
            await this._fillEditorFields(data.conditions, this.page.getByTestId('condition-list'), true);
        }
        if (data.actions) {
            await this._fillEditorFields(data.actions, this.page.getByTestId('action-list'));
        }
        if (data.splits) {
            ...
            for (const splitActions of data.splits) {
                ...
                await this._fillEditorFields(splitActions, this.page.getByTestId('action-list').nth(idx));
                idx++;
            }
        }
    }
