...
export class MobileBudgetPage {
    ...
    heading;
    previousMonthButton;
    selectedBudgetMonthButton;
    nextMonthButton;
    budgetPageMenuButton;
    budgetTableHeader;
    toBudgetButton;
    overbudgetedButton;
    savedButton;
    projectedSavingsButton;
    overspentButton;
    budgetedHeaderButton;
    spentHeaderButton;
    budgetTable;
    categoryRows;
    categoryNames;
    ...
    async openCategoryGroupMenu(categoryGroupName) {
        const categoryGroupButton = await this.#getButtonForCategoryGroup(categoryGroupName);
    ...
    async #getButtonForCell(buttonType, categoryName) {
        const buttonSelector = buttonType === 'Budgeted'
        ...
        throw new Error(`${buttonType} button for category ${categoryName} could not be located on the page.`);
    }
    ...
    async openBalanceMenu(categoryName) {
        ...
        if (await balanceButton.isVisible()) {
            ...
            throw new Error(`Balance button for category ${categoryName} not found or not visible.`);
        }
    }
    async #waitForNewMonthToLoad({ currentMonth, errorMessage, maxAttempts = 3, }) {
