...
test.describe('Onboarding', () => {
    ...
    test('creates a new budget file by importing YNAB4 budget', async () => {
        await configurationPage.clickOnNoServer();
        const budgetPage = await configurationPage.importBudget('YNAB4', path.resolve(__dirname, 'data/ynab4-demo-budget.zip'));
        await expect(budgetPage.budgetTable).toBeVisible({ timeout: 30000 });
        const accountPage = await navigation.goToAccountPage('Account1 with Starting Balance');
    ...
    test('creates a new budget file by importing nYNAB budget', async () => {
        await configurationPage.clickOnNoServer();
        const budgetPage = await configurationPage.importBudget('nYNAB', path.resolve(__dirname, 'data/ynab5-demo-budget.json'));
        await expect(budgetPage.budgetTable).toBeVisible({ timeout: 30000 });
    ...
    test('creates a new budget file by importing Actual budget', async () => {
        await configurationPage.clickOnNoServer();
        const budgetPage = await configurationPage.importBudget('Actual', path.resolve(__dirname, 'data/actual-demo-budget.zip'));
