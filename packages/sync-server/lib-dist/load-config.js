...
function resolveWebPath() {
    ...
    throw new Error('Could not resolve Actual web client directory. Set ACTUAL_WEB_DIR in .env or ensure @actual-app/web is installed.');
}
...
if (process.env.ACTUAL_CONFIG_PATH) {
    debug(`loading config from ACTUAL_CONFIG_PATH: '${process.env.ACTUAL_CONFIG_PATH}'`);
