 ...
 */
export const client = {
    getBalances: async (accountId) => await getGocardlessClient().account(accountId).getBalances(),
    getTransactions: async ({ accountId, dateFrom, dateTo }) => await getGocardlessClient().account(accountId).getTransactions({
        dateFrom,
        dateTo,
        country: undefined,
    }),
    getInstitutions: async (country) => await getGocardlessClient().institution.getInstitutions({ country }),
    getInstitutionById: async (institutionId) => await getGocardlessClient().institution.getInstitutionById(institutionId),
    getDetails: async (accountId) => await getGocardlessClient().account(accountId).getDetails(),
    getMetadata: async (accountId) => await getGocardlessClient().account(accountId).getMetadata(),
    getRequisitionById: async (requisitionId) => await getGocardlessClient().requisition.getRequisitionById(requisitionId),
    deleteRequisition: async (requisitionId) => await getGocardlessClient().requisition.deleteRequisition(requisitionId),
