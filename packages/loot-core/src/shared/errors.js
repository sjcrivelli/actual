    ...
    switch (reason) {
        ...
        case 'encrypt-failure':
            if (meta.isMissingKey) {
                return t('Encrypting your file failed because you are missing your encryption key. Create your key in the next step.');
            }
            return t('Encrypting the file failed. You have the correct key so this is an internal bug. To fix this, generate a new key in the next step.');
        case 'file-has-reset':
            // Something really weird happened - during reset a sanity
            // check on the server failed. The user just needs to
            // restart the whole process.
            return t('Something went wrong while resetting your file. Please try again.');
        case 'file-has-new-key':
            return t('Unable to encrypt your data because you are missing the key. Create the latest key in the next step.');
        case 'network':
...
export function getSyncError(error, id) {
    if (error === 'out-of-sync-migrations' || error === 'out-of-sync-data') {
        ...
        return t('Budget “{{id}}” not found. Check the ID of your budget in the Advanced section of the settings page.', { id });
    }
    else {
        return t('We had an unknown problem opening “{{id}}”.', { id });
    }
}
export function getBankSyncError(error) {
...
export function getUserAccessErrors(reason) {
    switch (reason) {
        case 'unauthorized':
            return t('You are not logged in.');
        case 'token-expired':
...
export function getSecretsError(error, reason) {
    switch (reason) {
        case 'unauthorized':
            return t('You are not logged in.');
        case 'not-admin':
...
export function getOpenIdErrors(reason) {
    switch (reason) {
        case 'unauthorized':
            return t('You are not logged in.');
        ...
        case 'unable-to-change-file-config-enabled':
            return t('Unable to enable OpenID. Please update the config.json file in this case.');
        default:
            return t('An internal error occurred, sorry! Visit https://actualbudget.org/contact/ for support. (ref: {{reason}})', { reason });
    }
}
