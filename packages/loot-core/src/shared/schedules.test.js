...
import { getRecurringDescription, getStatus, getUpcomingDays, } from './schedules';
i18next.init({
    lng: 'en',
    fallbackLng: 'en',
    resources: {
...
describe('schedules', () => {
    ...
    describe('getStatus', () => {
        ...
        it.each([1, 7, 14, 30])('returns upcoming if within upcoming range %n', (upcomingLength) => {
            ...
            expect(getStatus(tomorrow, false, false, upcomingLength.toString())).toBe('upcoming');
            expect(getStatus(upcomingDate, false, false, upcomingLength.toString())).toBe('upcoming');
            expect(getStatus(scheduledDate, false, false, upcomingLength.toString())).toBe('scheduled');
        });
        it('returns missed if past', () => {
            expect(getStatus(monthUtils.addDays(today, -1), false, false, '7')).toBe('missed');
        });
        it('returns scheduled if not due, upcoming, or missed', () => {
            expect(getStatus(monthUtils.addDays(today, 8), false, false, '7')).toBe('scheduled');
        });
    });
    describe('getRecurringDescription', () => {
        it('describes weekly interval', () => {
            expect(getRecurringDescription({ start: '2021-05-17', frequency: 'weekly' }, 'MM/dd/yyyy', enUS)).toBe('Every week on Monday');
            expect(getRecurringDescription({
                start: '2021-05-17',
                frequency: 'weekly',
                interval: 2,
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 weeks on Monday');
        });
        it('describes monthly interval', () => {
            expect(getRecurringDescription({ start: '2021-04-25', frequency: 'monthly' }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 25th');
            expect(getRecurringDescription({
                start: '2021-04-25',
                frequency: 'monthly',
                interval: 2,
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 months on the 25th');
            expect(getRecurringDescription({
                start: '2021-04-25',
                frequency: 'monthly',
                patterns: [{ type: 'day', value: 25 }],
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 25th');
            expect(getRecurringDescription({
                start: '2021-04-25',
                frequency: 'monthly',
                interval: 2,
            ...
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 31st');
            // -1 should work, representing the last day
            expect(getRecurringDescription({
                start: '2021-04-25',
                frequency: 'monthly',
                patterns: [{ type: 'day', value: -1 }],
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the last day');
            // Day names should work
            expect(getRecurringDescription({
                start: '2021-04-25',
                frequency: 'monthly',
                patterns: [{ type: 'FR', value: 2 }],
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 2nd Friday');
            expect(getRecurringDescription({
                start: '2021-04-25',
                frequency: 'monthly',
                patterns: [{ type: 'FR', value: -1 }],
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the last Friday');
        });
        it('describes monthly interval with multiple days', () => {
            ...
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 3rd, 15th, and 20th');
            expect(getRecurringDescription({
                start: '2021-04-25',
                frequency: 'monthly',
                patterns: [
            ...
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 3rd, 20th, and last day');
            ...
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 2nd Friday, 3rd, and last day');
            ...
            }, 'MM/dd/yyyy', enUS)).toBe('Every month on the 1st Saturday, 3rd Friday, 2nd, and 10th');
        });
        it('describes yearly interval', () => {
            expect(getRecurringDescription({ start: '2021-05-17', frequency: 'yearly' }, 'MM/dd/yyyy', enUS)).toBe('Every year on May 17th');
            expect(getRecurringDescription({
                start: '2021-05-17',
                frequency: 'yearly',
                interval: 2,
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 years on May 17th');
        });
        it('describes intervals with limited occurrences', () => {
            expect(getRecurringDescription({
                ...
                endOccurrences: 2,
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 weeks on Monday, 2 times');
            expect(getRecurringDescription({
                start: '2021-05-17',
                frequency: 'weekly',
                interval: 2,
                endMode: 'after_n_occurrences',
                endOccurrences: 1,
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 weeks on Monday, once');
            expect(getRecurringDescription({
                start: '2021-05-17',
                frequency: 'monthly',
                interval: 2,
                endMode: 'after_n_occurrences',
                endOccurrences: 2,
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 months on the 17th, 2 times');
            expect(getRecurringDescription({
                start: '2021-05-17',
                frequency: 'yearly',
                interval: 2,
                endMode: 'after_n_occurrences',
                endOccurrences: 2,
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 years on May 17th, 2 times');
        });
        it('describes intervals with an end date', () => {
            expect(getRecurringDescription({
                ...
                endMode: 'on_date',
                endDate: '2021-06-01',
            }, 'MM/dd/yyyy', enUS)).toBe('Every 2 weeks on Monday, until 06/01/2021');
            expect(getRecurringDescription({
                start: '2021-05-17',
                frequency: 'monthly',
                interval: 2,
                endMode: 'on_date',
                endDate: '2021-06-01',
            }, 'yyyy-MM-dd', enUS)).toBe('Every 2 months on the 17th, until 2021-06-01');
        });
    });
