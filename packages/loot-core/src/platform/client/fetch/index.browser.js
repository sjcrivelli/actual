    ...
    worker.onmessage = event => {
        ...
        if (msg.type === 'connect') {
            ...
            if (msg.message && msg.message.includes('indexeddb-quota-error')) {
                alert(t('We hit a limit on the local storage available. Edits may not be saved. Please get in touch https://actualbudget.org/contact/ so we can help debug this.'));
            }
        }
...
export const send = function (...params) {
...
export const serverPush = async function () { };
export const clearServer = async function () { };
