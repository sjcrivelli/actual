...
async function expectPagedData(query, numTransactions, allData) {
    ...
    do {
        ...
        const { data } = await aqlQuery(query.limit(pageCount).offset(pagedData.length).serialize());
...
describe('transaction executors', () => {
    it('queries with `splits: inline` returns only non-parents', async () => {
        await fc.assert(fc.asyncProperty(arbs.makeTransactionArray({
            splitFreq: 2,
            minLength: 2,
            maxLength: 20,
        }), async (arr) => {
            ...
            const { data: defaultData } = await aqlQuery(q('transactions')
                .filter({ amount: { $lt: 0 } })
                .select('*')
                .serialize());
