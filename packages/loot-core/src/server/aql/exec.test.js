...
describe('compileAndRunQuery', () => {
    it('converts output types', async () => {
        ...
        // date
        let { data } = await compileAndRunAqlQuery(q('transactions').select('date').serialize());
        expect(data[0].date).toBe('2020-01-04');
        // date-month
        data = (await compileAndRunAqlQuery(q('transactions')
        ...
        // boolean
        data = (await compileAndRunAqlQuery(q('transactions').select(['is_child', 'is_parent']).raw().serialize())).data;
    ...
    it('provides named parameters and converts types', async () => {
        ...
        let { data } = await compileAndRunAqlQuery(q('transactions')
    ...
    it('parameters have the correct order', async () => {
        ...
        const { data } = await compileAndRunAqlQuery(q('transactions')
            .filter({
