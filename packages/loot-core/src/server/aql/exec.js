...
export async function execQuery(queryState, compilerState, sqlPieces, params, outputTypes) {
    const sql = defaultConstructQuery(queryState, compilerState, sqlPieces);
    const data = await db.all(sql, params);
    applyTypes(data, outputTypes);
    return data;
}
export async function runCompiledAqlQuery(queryState, sqlPieces, compilerState, { params = {}, executors = {} } = {}) {
...
export async function compileAndRunAqlQuery(schema, schemaConfig, queryState, options) {
