...
import { APIError, BankSyncError, PostError, TransactionError, } from '../errors';
...
async function getAccountBalance({ id, cutoff, }) {
    const result = await db.first('SELECT sum(amount) as balance FROM transactions WHERE acct = ? AND isParent = 0 AND tombstone = 0 AND date <= ?', [id, db.toDateRepr(dayFromDate(cutoff))]);
...
function handleSyncError(err, acct) {
    ...
    if (err instanceof BankSyncError || err?.type === 'BankSyncError') {
        const error = err;
        const syncError = {
            type: 'SyncError',
            accountId: acct.id,
