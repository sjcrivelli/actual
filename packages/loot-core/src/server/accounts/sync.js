...
import { makeChild as makeChildTransaction, recalculateSplit, } from '../../shared/transactions';
import { hasFieldsChanged, amountToInteger, integerToAmount, } from '../../shared/util';
...
import { defaultMappings, mappingsFromString, } from '../util/custom-sync-mapping';
...
function makeSplitTransaction(trans, subtransactions) {
    ...
    const { subtransactions: sub, ...parent } = recalculateSplit({
        ...trans,
        is_parent: true,
        subtransactions: subtransactions.map((transaction, idx) => makeChildTransaction(trans, {
            ...transaction,
            sort_order: 0 - idx,
        })),
    });
...
async function getAccountOldestTransaction(id) {
    return (await aqlQuery(q('transactions')
        .filter({
        account: id,
...
export async function getGoCardlessAccounts(userId, userKey, id) {
    ...
    if (!userToken)
        return;
    const res = await post(getServer().GOCARDLESS_SERVER + '/accounts', {
        userId,
        key: userKey,
        item_id: id,
    }, {
...
export async function reconcileTransactions(acctId, transactions, isBankSyncAccount = false, strictIdChecking = true, isPreview = false, defaultCleared = true) {
...
export async function matchTransactions(acctId, transactions, isBankSyncAccount = false, strictIdChecking = true) {
    logger.log('Performing transaction reconciliation matching');
    const reimportDeleted = await aqlQuery(q('preferences')
...
export async function addTransactions(acctId, transactions, { runTransfers = true, learnCategories = false } = {}) {
    const added = [];
...
async function processBankSyncDownload(download, id, acctRow, initialSync = false) {
    ...
    const importTransactions = await aqlQuery(q('preferences')
...
export async function simpleFinBatchSync(accounts) {
