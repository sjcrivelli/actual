...
import { extractScheduleConds, getDateWithSkippedWeekend, getHasTransactionsQuery, getNextDate, getScheduledAmount, getStatus, recurConfigToRSchedule, } from '../../shared/schedules';
...
import { getRules, insertRule, ruleModel, updateRule, } from '../transactions/transaction-rules';
...
export async function getRuleForSchedule(id) {
    ...
    const { data: ruleId } = await aqlQuery(q('schedules').filter({ id }).calculate('rule'));
...
export async function setNextDate({ id, start, conditions, reset, }) {
    ...
    const { data: nextDate } = await aqlQuery(q('schedules').filter({ id }).calculate('next_date'));
...
export async function createSchedule({ schedule = null, conditions = [], } = {}) {
...
export async function updateSchedule({ schedule, conditions, resetNextDate, }) {
    ...
    await batchMessages(async () => {
        if (conditions) {
            ...
            if (resetNextDate ||
                !deepEqual(oldConditions.find(c => c.field === 'account'), oldConditions.find(c => c.field === 'account')) ||
                !deepEqual(stripType(oldConditions.find(c => c.field === 'date') || {}), stripType(newConditions.find(c => c.field === 'date') || {}))) {
                await setNextDate({
                    id: schedule.id,
...
async function advanceSchedulesService(syncSuccess) {
    ...
    const { data: schedules } = await aqlQuery(q('schedules')
    ...
    const failedToPost = [];
    let didPost = false;
    const { data: upcomingLength } = await aqlQuery(q('preferences')
...
app.method('schedule/post-transaction', mutator(undoable(postTransactionForSchedule)));
app.method('schedule/force-run-service', mutator(() => advanceSchedulesService(true)));
