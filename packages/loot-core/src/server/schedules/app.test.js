...
import { updateConditions, createSchedule, updateSchedule, deleteSchedule, setNextDate, } from './app';
beforeEach(async () => {
...
describe('schedule app', () => {
    describe('utility', () => {
        ...
        it('getNextDate works with date conditions', () => {
            expect(getNextDate({ op: 'is', field: 'date', value: '2021-04-30' })).toBe('2021-04-30');
            expect(getNextDate({
                op: 'is',
                field: 'date',
                value: {
    ...
    describe('methods', () => {
        it('createSchedule creates a schedule', async () => {
            ...
            const { data: [row], } = await aqlQuery(q('schedules').filter({ id }).select('*'));
        ...
        it('updateSchedule updates a schedule', async () => {
            ...
            let res = await aqlQuery(q('schedules')
                .filter({ id })
            ...
            res = await aqlQuery(q('schedules')
                .filter({ id })
                .select(['next_date', 'posts_transaction']));
            row = res.data[0];
        ...
        it('setNextDate sets `next_date`', async () => {
            ...
            let res = await aqlQuery(q('schedules').filter({ id }).select(['next_date']));
            let row = res.data[0];
            expect(row.next_date).toBe('2020-12-30');
