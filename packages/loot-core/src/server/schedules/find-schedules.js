...
async function weekly(startDate, accountId) {
    return schedulesForPattern(d.subWeeks(parseDate(startDate), 4), 7 * 2, { frequency: 'weekly' }, accountId);
}
async function every2weeks(startDate, accountId) {
    ...
    // week back
    d.subWeeks(parseDate(startDate), 7), 7 * 2, { frequency: 'weekly', interval: 2 }, accountId);
}
async function monthly(startDate, accountId) {
    return schedulesForPattern(d.subMonths(parseDate(startDate), 4), 31 * 2, start => {
...
async function monthlyLastDay(startDate, accountId) {
    ...
    const s1 = await schedulesForPattern(d.subMonths(parseDate(startDate), 3), 1, { frequency: 'monthly', patterns: [{ type: 'day', value: -1 }] }, accountId);
    const s2 = await schedulesForPattern(d.subMonths(parseDate(startDate), 4), 1, { frequency: 'monthly', patterns: [{ type: 'day', value: -1 }] }, accountId);
    return s1.concat(s2);
}
async function monthly1stor3rd(startDate, accountId) {
    return schedulesForPattern(d.subWeeks(parseDate(startDate), 8), 14, start => {
...
async function monthly2ndor4th(startDate, accountId) {
    return schedulesForPattern(d.subMonths(parseDate(startDate), 8), 14, start => {
        const day = d.format(new Date(), 'iiii');
        const dayValue = day.slice(0, 2).toUpperCase();
        return {
            start,
...
async function findStartDate(schedule) {
    ...
    while (1) {
        ...
        switch (currentConfig.frequency) {
            case 'weekly':
                currentConfig.start = dayFromDate(d.subWeeks(parseDate(currentConfig.start), currentConfig.interval || 1));
                break;
            case 'monthly':
                currentConfig.start = dayFromDate(d.subMonths(parseDate(currentConfig.start), currentConfig.interval || 1));
                break;
            case 'yearly':
                currentConfig.start = dayFromDate(d.subYears(parseDate(currentConfig.start), currentConfig.interval || 1));
                break;
            default:
...
export async function findSchedules() {
    ...
    const schedules = [...groupBy(allSchedules, 'payee').entries()].map(([, schedules]) => {
