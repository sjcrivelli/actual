...
async function updateDashboard(widgets) {
    const { data: dbWidgets } = await aqlQuery(q('dashboard')
...
async function resetDashboard() {
    await batchMessages(async () => {
        await Promise.all([
            ...
            ...DEFAULT_DASHBOARD_STATE.map(widget => db.insertWithSchema('dashboard', widget)),
        ]);
    });
}
        ...
        await batchMessages(async () => {
            await Promise.all([
                ...
                ...parsedContent.widgets
                    ...
                    .map(({ meta }) => db.insertWithSchema('custom_reports', reportModel.fromJS(meta))),
