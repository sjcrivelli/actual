...
describe('Merging fails for invalid quantity', () => {
    ...
    tests.forEach(([arr, message]) => it(message, () => expect(() => mergeTransactions(arr)).rejects.toThrow()));
    it("fails when amounts don't match", async () => {
        ...
        expect(() => mergeTransactions([{ id: t1 }, { id: t2 }])).rejects.toThrow('Transaction amounts must match for merge');
    });
    it("fails when transaction id doesn't exist", async () => {
        ...
        expect(() => mergeTransactions([{ id: t1 }, { id: 'missing' }])).rejects.toThrow('One of the provided transactions does not exist');
    });
});
