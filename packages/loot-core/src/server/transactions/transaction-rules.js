...
import { Condition, Action, Rule, RuleIndexer, rankRules, migrateIds, iterateIds, execActions, } from '../rules';
...
export async function getRuleIdFromScheduleId(scheduleId) {
    const row = await db.first('SELECT rule FROM schedules WHERE id = ?', [scheduleId]);
    return row?.rule || null;
}
export async function getAllRuleIdsFromSchedules(excluding) {
...
// Runner
export async function runRules(trans, accounts = null) {
...
export function conditionsToAQL(conditions, { recurDateBounds = 100, applySpecialCases = true } = {}) {
    const errors = [];
...
export async function applyActions(transactions, actions) {
...
export async function prepareTransactionForRules(trans, accounts = null) {
    const r = { ...trans };
    if (trans.payee) {
...
export async function finalizeTransactionForRules(trans) {
