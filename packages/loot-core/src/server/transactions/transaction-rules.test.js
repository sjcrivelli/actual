...
import { getRules, loadRules, insertRule, updateRule, deleteRule, makeRule, runRules, conditionsToAQL, resetState, getProbableCategory, updateCategoryRules, } from './transaction-rules';
...
describe('Learning categories', () => {
    ...
    async function insertTransaction(transaction, expectedCategory, expectedRuleCount = 1, expectedPayee = 'foo') {
        ...
        if (expectedRuleCount > 0) {
            expectCategoryRule(getRules()[expectedRuleCount - 1], expectedCategory, expectedPayee);
        }
    }
