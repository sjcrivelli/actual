...
async function idsWithChildren(ids) {
    ...
    const rows = await db.all(`SELECT id FROM v_transactions_internal WHERE ${whereIds}`);
...
async function getTransactionsByIds(ids) {
    ...
    return incrFetch((query, params) => db.selectWithSchema('transactions', query, params), ids, 
    // eslint-disable-next-line actual/typography
    id => `id = '${id}'`, where => `SELECT * FROM v_transactions_internal WHERE ${where}`);
}
export async function batchUpdateTransactions({ added, deleted, updated, learnCategories = false, detectOrphanPayees = true, runTransfers = true, }) {
    ...
    await batchMessages(async () => {
        if (added) {
            addedIds = await Promise.all(added.map(async (t) => {
    ...
    if (runTransfers) {
        await batchMessages(async () => {
            ...
            transfersUpdated = (await Promise.all(allUpdated.map(t => transfer.onUpdate(t)))).filter(Boolean);
    ...
    if (learnCategories) {
        ...
        await rules.updateCategoryRules(allAdded.concat(allUpdated).filter(trans => ids.has(trans.id)));
    }
