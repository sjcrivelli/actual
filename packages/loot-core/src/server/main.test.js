...
import { runHandler, runMutator, disableGlobalMutations, enableGlobalMutations, } from './mutators';
...
describe('Accounts', () => {
    test('Transfers are properly updated', async () => {
        ...
        const differ = expectSnapshotWithDiffer(await db.all('SELECT * FROM transactions'));
...
describe('Budget', () => {
    ...
    test('budget updates when changing a category', async () => {
        ...
        await captureChangedCells(async () => {
            await runMutator(() => db.insertCategoryGroup({ id: 'group1', name: 'group1' }));
            categories = [
        ...
        // Test insertions
        let changed = await captureChangedCells(() => runHandler(handlers['transaction-add'], trans));
