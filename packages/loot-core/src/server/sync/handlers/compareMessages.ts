...
import type { SyncMessageDiff, CompareResult } from '../types';
...
export function compareMessages(
  localMessages: SyncMessageDiff[],
  remoteMessages: SyncMessageDiff[]
): CompareResult {
  log.info(
    `[compareMessages] Comparing ${localMessages.length} local ↔ ${remoteMessages.length} remote`
  );

  const newMessages: SyncMessageDiff[] = [];
  const updatedMessages: SyncMessageDiff[] = [];
  ...
  log.debug(
    `[compareMessages] Found ${newMessages.length} new and ${updatedMessages.length} updated`
  );
