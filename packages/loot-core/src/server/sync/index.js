// @ts-strict-ignore
import { serializeClock, deserializeClock, getClock, Timestamp, merkle, } from '@actual-app/crdt';
...
export const applyMessages = sequential(async (messages) => {
    ...
    db.transaction(() => {
        ...
        if (checkSyncingMode('enabled')) {
            ...
            // exceptions)
            db.runQuery(db.cache('INSERT OR REPLACE INTO messages_clock (id, clock) VALUES (1, ?)'), [serializeClock({ ...clock, merkle: currentMerkle })]);
        }
    });
    if (checkSyncingMode('enabled')) {
