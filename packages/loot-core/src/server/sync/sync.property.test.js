...
const uuidGenerator = jsc.integer(97, 122).smap(x => String.fromCharCode(x), x => x.charCodeAt(Number(x)));
beforeEach(() => {
...
function makeGen({ table, row, field, value, }) {
    return jsc.record({
        ...
        value,
        timestamp: jsc.integer(1000, 10000).smap(x => {
            let clientId;
        ...
        switch (type) {
            ...
            case 'integer':
                if (field === 'amount') {
                    ...
                    generators.push(makeGen({ table, field, value: jsc.elements([0, 1]) }));
                }
                break;
            case 'real':
...
async function run(msgs) {
    ...
    prefs.savePrefs({
        groupId: 'group',
        lastSyncedTimestamp: new Timestamp(Date.now(), 0, '0000000000000000').toString(),
    });
    ...
    await client1Sync;
    await mockSyncServer.handlers['/sync/sync'](await encoder.encode('group', clientId2, Timestamp.zero, res.secondMessages.map(x => ({
        ...x,
    ...
    mockSyncServer.handlers['/sync/sync'](await encoder.encode('group', clientId2, Timestamp.zero, res.secondMessages.map(x => ({
        ...x,
        value: sync.serializeValue(x.value),
        timestamp: x.timestamp.toString(),
    }))));
...
describe('sync property test', () => {
    it.skip('should always sync clients into the same state', async () => {
        const test = await jsc.check(jsc.forall(jsc.tuple(Array.from(new Array(100)).map(() => jsc.oneof(generators))), async (msgs) => {
            let r;
            try {
                r = await run(msgs);
            }
