...
import { getNextDate, getDateWithSkippedWeekend, extractScheduleConds, } from '../../shared/schedules';
...
async function createScheduleList(templates, current_month, category) {
    ...
    for (const template of templates) {
        ...
        const next_date_string = getNextDate(dateConditions, monthUtils._parse(current_month));
        ...
        if (num_months < 0) {
            ...
            if (!completed) {
                if (isRepeating) {
                    ...
                    let nextBaseDate = getNextDate(dateConditions, monthUtils._parse(current_month), true);
                    let nextDate = dateConditions.value.skipWeekend
                        ? monthUtils.dayFromDate(getDateWithSkippedWeekend(monthUtils._parse(nextBaseDate), dateConditions.value.weekendSolveMode))
                        : nextBaseDate;
                    while (nextDate < nextMonth) {
                        monthlyTarget += -target;
                        const currentDate = nextBaseDate;
                        const oneDayLater = monthUtils.addDays(nextBaseDate, 1);
                        nextBaseDate = getNextDate(dateConditions, monthUtils._parse(oneDayLater), true);
...
export async function runSchedule(template_lines, current_month, balance, remainder, last_month_balance, to_budget, errors, category) {
