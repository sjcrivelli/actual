...
const rules_1 = require("../server/rules");
...
function getRecurringDescription(config, dateFormat, locale) {
    ...
    switch (config.frequency) {
        case 'daily':
            desc =
                interval !== 1
                    ? (0, i18next_1.t)(`Every {{interval}} days`, { interval })
        ...
        case 'monthly':
            if (config.patterns && config.patterns.length > 0) {
                ...
                if (strs.length > 2) {
                    ...
                    range += `, ${(0, i18next_1.t)('and')} `;
                    range += strs[strs.length - 1];
                }
                else {
                    range += strs.join(` ${(0, i18next_1.t)('and')} `);
                }
                if (isSameDay) {
                ...
                desc =
                    interval !== 1
                        ...
                        : (0, i18next_1.t)('Every month on the {{range}}', { range });
            }
            else {
                desc =
                    interval !== 1
...
function getNextDate(dateCond, start = new Date(monthUtils.currentDay()), noSkipWeekend = false) {
    start = d.startOfDay(start);
    const cond = new rules_1.Condition(dateCond.op, 'date', dateCond.value, null);
...
function describeSchedule(schedule, payee) {
    if (payee) {
        ...
        return `${(0, i18next_1.t)('Next:')} ${schedule.next_date}`;
    }
}
function getUpcomingDays(upcomingLength = '7', today = monthUtils.currentDay()) {
