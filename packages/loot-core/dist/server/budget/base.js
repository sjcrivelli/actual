...
const budgetActions = __importStar(require("./actions"));
const envelopeBudget = __importStar(require("./envelope"));
...
function createCategory(cat, sheetName, prevSheetName, start, end) {
    sheet.get().createDynamic(sheetName, 'sum-amount-' + cat.id, {
        ...
        run: () => {
            ...
            const rows = db.runQuery(`SELECT SUM(amount) as amount FROM v_transactions_internal_alive t
...
function handleBudgetChange(budget) {
    if (budget.category) {
        ...
        sheet
            .get()
            .set(`${sheetName}!carryover-${budget.category}`, budget.carryover === 1 ? true : false);
        ...
        newValues.forEach((items, table) => {
            ...
            items.forEach(newValue => {
                ...
                if (table === 'zero_budget_months') {
                    ...
                    const changed = new Set(Object.keys((0, util_1.getChangedValues)(oldValue || {}, newValue) || {}));
                    if (oldValue) {
...
async function createBudget(months) {
    ...
    months.forEach(month => {
        if (!meta.createdMonths.has(month)) {
            ...
            if (budgetType === 'envelope') {
                envelopeBudget.createSummary(groups, categories, prevSheetName, sheetName);
            }
            else {
...
async function createAllBudgets() {
    const earliestTransaction = await db.first('SELECT * FROM transactions WHERE isChild=0 AND date IS NOT NULL ORDER BY date ASC LIMIT 1');
