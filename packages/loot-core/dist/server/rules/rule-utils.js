...
function computeScore(rule) {
    const initialScore = rule.conditions.reduce((score, condition) => {
        if (OP_SCORES[condition.op] == null) {
            log_1.logger.log(`Found invalid operation while ranking: ${condition.op}`);
            return 0;
        }
...
function parseRecurDate(desc) {
    try {
        const rules = (0, schedules_1.recurConfigToRSchedule)(desc);
        return {
            type: 'recur',
        ...
        throw new errors_1.RuleError('parse-recur-date', getErrorMessage(e));
    }
}
