...
async function prepareDatabase() {
    ...
    const { accounts } = await (0, post_1.post)((0, server_config_1.getServer)().GOCARDLESS_SERVER + '/accounts', {
        client_id: '',
        group_id: '',
        item_id: '1',
    });
...
describe('Account sync', () => {
    test('reconcile creates payees correctly', async () => {
        ...
        await (0, sync_1.reconcileTransactions)(id, [
        ...
        expect(transactions.find(t => t.amount === 4133).payee).toBe(payees.find(p => p.name === 'Bakkerij').id);
        expect(transactions.find(t => t.amount === 5000).payee).toBe(payees.find(p => p.name === 'Kroger').id);
    });
    test('reconcile handles transactions with undefined fields', async () => {
        ...
        await (0, sync_1.reconcileTransactions)(acctId, [
            { date: '2020-01-02' },
        ...
        await expect((0, sync_1.reconcileTransactions)(acctId, [{}])).rejects.toThrow(/`date` is required/);
    });
    test('reconcile doesnt rematch deleted transactions if reimport disabled', async () => {
        const { id: acctId } = await prepareDatabase();
        const reimportKey = `sync-reimport-deleted-${acctId}`;
    ...
    const testMapped = version => {
        test(`reconcile matches unmapped and mapped payees (${version})`, async () => {
            ...
            const { updated } = await (0, sync_1.reconcileTransactions)(acctId, [
                {
                    date: '2017-10-17',
                    payee_name: 'bakkerij',
                    amount: -2947,
            ...
            const transactions = await getAllTransactions();
            expect(transactions.length).toBe(2);
            expect(transactions.find(t => t.id === 'one').imported_id).toBe('imported1');
        });
    };
    testMapped('v1');
    testMapped('v2');
    test('addTransactions simply adds transactions', async () => {
        ...
        const added = await (0, sync_1.addTransactions)(acctId, transactions);
    ...
    test('reconcile does not merge transactions with different ‘imported_id’ values', async () => {
        ...
        expect(transactions.length).toBe(2);
        expect(transactions.find(t => t.imported_id === 'b85cdd57-5a1c-4ca5-bd54-12e5b56fa02c').amount).toBe(-1239);
    ...
    test('given an imported tx with no imported_id, ' +
        ...
        'then imported tx should reconcile with existing transaction from fuzzy match', async () => {
        const { id } = await prepareDatabase();
        let payees = await getAllPayees();
        expect(payees.length).toBe(0);
        const existingTx = {
        ...
        await (0, sync_1.reconcileTransactions)(id, [existingTx]);
        payees = await getAllPayees();
        expect(payees.length).toBe(1);
        let transactions = await getAllTransactions();
        expect(transactions.length).toBe(1);
