...
describe('sheet language', () => {
    it('`select` should select fields', () => {
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('accounts')
    ...
    it('`like` should use unicode and normalise function', () => {
        const result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions')
            .select('payee')
    ...
    it('`select` allows selecting all fields with *', () => {
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('accounts').select(['*']).serialize(), schemaWithRefs);
        expect(sqlLines(result.sql)).toEqual(sqlLines(`
    ...
    it('`select` excludes deleted rows by default and `withDead` includes them', () => {
        ...
        // doesn't have it )
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('accounts').select(['trans']).withoutValidatedRefs().serialize(), schemaWithTombstone);
    ...
    it('`select` always includes the id', () => {
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('payees').select('name').serialize(), schemaWithRefs);
    ...
    it('automatically joins tables if referenced by path', () => {
        // Join a simple table
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions')
    ...
    it('avoids unnecessary joins when deeply joining', () => {
        const { state, sql } = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions')
            .filter({
    ...
    it('groupBy should work', () => {
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions').groupBy('payee.name').select('id').serialize(), schemaWithRefs);
        expect(sqlLines(result.sql)).toEqual(sqlLines(`
    ...
    it('orderBy should work', () => {
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions').orderBy('payee.name').select('id').serialize(), schemaWithRefs);
        expect(sqlLines(result.sql)).toEqual(sqlLines(`
        SELECT transactions.id AS id FROM transactions
        LEFT JOIN payees payees1 ON payees1.id = transactions.payee
        WHERE 1
    ...
    it('allows limit and offset', () => {
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions').select(['id']).limit(10).serialize(), schemaWithRefs);
    ...
    it('allows customizing generated SQL', () => {
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions').select(['amount']).serialize(), schemaWithRefs, {
      ...
      `));
        ...
        expect(() => (0, compiler_1.generateSQLWithState)((0, query_1.q)('accounts').select(['trans1.amount']).serialize(), schemaWithRefs, {
            tableViews: { transactions: 'v_transactions' },
            tableFilters: name => name === 'transactions' ? [{ 'payee.name': 'foo' }] : [],
        })).toThrow(/cannot contain paths/);
    });
    it('raw mode avoids any internal filters', () => {
        const result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions').select(['amount']).raw().serialize(), schemaWithRefs, {
            tableViews: { transactions: 'v_transactions' },
            tableFilters: name => name === 'transactions' ? [{ amount: { $gt: 0 } }] : [],
        });
        expect(sqlLines(result.sql)).toEqual(sqlLines(`
    ...
    it('tracks compiler state for debugging', () => {
        ...
        try {
            ...
            expect(getErrorMessage(e)).toMatch('filter({"date":{"$transform":"$month","$eq":10}})');
        }
        // group by
        try {
            (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions')
...
describe('Type conversions', () => {
    ...
    it('allows fields to be nullable', () => {
        // With validated refs
        let result = (0, compiler_1.generateSQLWithState)((0, query_1.q)('transactions').filter({ payee: null }).select().serialize(), schemaWithRefs);
