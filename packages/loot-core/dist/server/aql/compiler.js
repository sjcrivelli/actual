...
const compileOp = saveStack('op', (state, fieldRef, opData) => {
    ...
    switch (op) {
        ...
        case '$like': {
            ...
            return `UNICODE_LIKE(${(0, normalisation_1.getNormalisedString)(right)}, NORMALISE(${left}))`;
        }
        case '$regexp': {
            const [left, right] = valArray(state, [lhs, rhs], ['string', 'string']);
        ...
        case '$notlike': {
            const [left, right] = valArray(state, [lhs, rhs], ['string', 'string']);
            return `(NOT UNICODE_LIKE(${(0, normalisation_1.getNormalisedString)(right)}, NORMALISE(${left}))\n OR ${left} IS NULL)`;
        }
        default:
