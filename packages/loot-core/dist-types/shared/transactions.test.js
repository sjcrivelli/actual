...
function makeSplitTransaction(data, children) {
    ...
    return [parent, ...children.map(t => (0, transactions_1.makeChild)(parent, t))];
}
...
describe('Transactions', () => {
    test('updating a transaction works', () => {
        ...
        const { data, diff } = (0, transactions_1.updateTransaction)(transactions, makeTransaction({
            id: 't1',
            amount: 5000,
        }));
    ...
    test('updating does nothing if value not changed', () => {
        ...
        const { data, diff } = (0, transactions_1.updateTransaction)(transactions, updatedTransaction);
    ...
    test('deleting a transaction works', () => {
        ...
        const { data, diff } = (0, transactions_1.deleteTransaction)(transactions, 't1');
        expect(diff).toEqual({
            added: [],
            deleted: [{ id: 't1' }],
            updated: [],
    ...
    test('splitting a transaction works', () => {
        ...
        const { data, diff } = (0, transactions_1.splitTransaction)(transactions, 't1');
        expect(data.find(d => d.subtransactions)).toBeFalsy();
        expect(diff).toEqual({
    ...
    test('adding a split transaction works', () => {
        ...
        const { data, diff } = (0, transactions_1.addSplitTransaction)(transactions, 't1');
        expect(data.find(d => d.subtransactions)).toBeFalsy();
    ...
    test('deleting a split transaction works', () => {
        ...
        const { data, diff } = (0, transactions_1.deleteTransaction)(transactions, 't2');
        expect(diff).toEqual({
            added: [],
    ...
    test('deleting all child split transactions works', () => {
        const transactions = [
            makeTransaction({ amount: 2001 }),
            ...makeSplitTransaction({ id: 't1', amount: 2500, error: splitError(500) }, [{ id: 't2', amount: 2000 }]),
            makeTransaction({ amount: 3002 }),
        ];
        const { data } = (0, transactions_1.deleteTransaction)(transactions, 't2');
        expect(data).toEqual([
            expect.objectContaining({ amount: 2001 }),
