...
async function createPayee(description) {
    ...
    // name
    const row = await db.first(`SELECT id FROM payees WHERE UNICODE_LOWER(name) = ? AND tombstone = 0`, [description.toLowerCase()]);
    if (row) {
        return row.id;
    }
    else {
        return (await db.insertPayee({ name: description }));
    }
}
async function getStartingBalancePayee() {
  ...
  `);
    if (category === null) {
        category = await db.first('SELECT * FROM categories WHERE is_income = 1 AND tombstone = 0');
    }
