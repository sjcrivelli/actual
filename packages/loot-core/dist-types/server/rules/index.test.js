...
describe('Condition', () => {
    ...
    test('ops handles null fields', () => {
        let cond = new _1.Condition('contains', 'notes', 'foo', null);
        ...
        ['gt', 'gte', 'lt', 'lte', 'isapprox'].forEach(op => {
            const cond = new _1.Condition(op, 'date', '2020-01-01', null);
    ...
    test('ops handles undefined fields', () => {
        ...
        let cond = new _1.Condition('is', 'payee', null, null);
    ...
    test('date conditions work with `is` operator', () => {
        let cond = new _1.Condition('is', 'date', '2020-08-10', null);
    ...
    test('recurring date conditions work with `is` operator', () => {
        let cond = new _1.Condition('is', 'date', {
            start: '2019-01-01',
            frequency: 'monthly',
    ...
    test('date conditions work with comparison operators', () => {
        let cond = new _1.Condition('gt', 'date', '2020-08-10', null);
    ...
    test('id works with all operators', () => {
        let cond = new _1.Condition('is', 'payee', 'foo', null);
    ...
    test('string works with all operators', () => {
        let cond = new _1.Condition('is', 'notes', 'foo', null);
    ...
    test('matches handles invalid regex', () => {
        const cond = new _1.Condition('matches', 'notes', 'fo**', null);
    ...
    test('number works with all operators', () => {
        let cond = new _1.Condition('is', 'amount', 155, null);
...
describe('Action', () => {
    test('`set` operator sets a field', () => {
        const action = new _1.Action('set', 'notes', 'James', null);
    ...
    describe('templating', () => {
        test('should use available fields', () => {
            const action = new _1.Action('set', 'notes', '', {
        ...
        describe('regex helper', () => {
            function testHelper(template, expected) {
                test(template, () => {
                    const action = new _1.Action('set', 'notes', '', { template });
        ...
        describe('math helpers', () => {
            function testHelper(template, expected, field = 'amount') {
                test(template, () => {
                    const action = new _1.Action('set', field, '', { template });
                    const item = { [field]: 10 };
                    action.exec(item);
...
describe('Rule', () => {
    ...
    describe('split actions', () => {
        ...
        const fixedAmountRule = new _1.Rule({
            conditionsOp: 'and',
        ...
        const prioritizationRule = new _1.Rule({
            conditionsOp: 'and',
            conditions: [{ op: 'is', field: 'imported_payee', value: 'James' }],
            actions: [
                {
        ...
        test('remainder rounds correctly and only if necessary', () => {
            ...
            expect(rule.exec({ imported_payee: 'James', amount: -2397 })).toMatchObject({
    ...
    test('rules are deterministically ranked', () => {
        const rule = (id, conditions) => new _1.Rule({
            id,
            conditionsOp: 'and',
            conditions,
            actions: [],
    ...
    test('iterateIds finds all the ids', () => {
        const rule = (id, conditions, actions = []) => new _1.Rule({ id, conditionsOp: 'and', conditions, actions });
        const rules = [
        ...
        const foundRules = [];
        (0, _1.iterateIds)(rules, 'payee', rule => {
...
describe('RuleIndexer', () => {
    test('indexing a single field works', () => {
        const indexer = new _1.RuleIndexer({ field: 'notes' });
        const rule = new _1.Rule({
            conditionsOp: 'and',
            conditions: [{ op: 'is', field: 'notes', value: 'James' }],
            actions: [{ op: 'set', field: 'notes', value: 'Sarah' }],
        ...
        // needs to be run
        expect(indexer.getApplicableRules({ notes: 'James' })).toEqual(new Set([rule, rule2]));
    ...
    test('indexing using the firstchar method works', () => {
        ...
        const indexer = new _1.RuleIndexer({ field: 'category', method: 'firstchar' });
        const rule = new _1.Rule({
            conditionsOp: 'and',
            conditions: [
                { op: 'is', field: 'notes', value: 'James' },
        ...
        expect(indexer.getApplicableRules({ notes: 'James', category: 'food' })).toEqual(new Set([rule, rule3]));
        expect(indexer.getApplicableRules({ notes: 'James', category: 'f' })).toEqual(new Set([rule, rule3]));
        expect(indexer.getApplicableRules({ notes: 'James', category: 'foo' })).toEqual(new Set([rule, rule3]));
    ...
    test('indexing works with the oneOf operator', () => {
        const indexer = new _1.RuleIndexer({ field: 'notes', method: 'firstchar' });
        const rule = new _1.Rule({
            conditionsOp: 'and',
            conditions: [
        ...
        indexer.index(rule2);
        expect(indexer.getApplicableRules({ notes: 'James' })).toEqual(new Set([rule]));
        expect(indexer.getApplicableRules({ notes: 'Evy' })).toEqual(new Set([rule]));
        expect(indexer.getApplicableRules({ notes: 'Charlotte' })).toEqual(new Set([]));
        expect(indexer.getApplicableRules({ notes: 'Georgia' })).toEqual(new Set([rule2]));
    });
});
