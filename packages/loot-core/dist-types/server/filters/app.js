...
const filterModel = {
    ...
    toJS(row) {
        ...
        return {
            ...
            conditions: (0, transaction_rules_1.parseConditionsOrActions)(conditions),
        };
    },
    fromJS(filter) {
...
async function filterNameExists(name, filterId, newItem) {
    const idForName = await db.first('SELECT id from transaction_filters WHERE tombstone = 0 AND name = ?', [name]);
    if (idForName === null) {
        return false;
    }
    if (!newItem) {
...
function conditionExists(item, filters, newItem) {
    ...
    filters.some(filter => {
        if ((conditions.length === 1 || filter.conditionsOp === conditionsOp) &&
            ...
            const allConditionsMatch = !conditions.some(cond => !filter.conditions.some(fcond => cond.value === fcond.value &&
                cond.op === fcond.op &&
