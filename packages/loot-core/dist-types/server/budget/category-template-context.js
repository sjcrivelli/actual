...
// @ts-strict-ignore
const query_1 = require("loot-core/shared/query");
...
class CategoryTemplateContext {
    ...
    static async init(templates, category, month, budgeted) {
        ...
        const lastMonthSheet = monthUtils.sheetForMonth(monthUtils.subMonths(month, 1));
        const lastMonthBalance = await (0, actions_1.getSheetValue)(lastMonthSheet, `leftover-${category.id}`);
        const carryover = await (0, actions_1.getSheetBoolean)(lastMonthSheet, `carryover-${category.id}`);
        let fromLastMonth;
        ...
        const hideDecimal = await (0, aql_1.aqlQuery)((0, query_1.q)('preferences').filter({ id: 'hideFraction' }).select('*'));
        // call the private constructor
        return new CategoryTemplateContext(templates, category, month, fromLastMonth, budgeted, hideDecimal.data.length > 0
            ...
            switch (template.type) {
                ...
                case 'schedule': {
                    if (!scheduleFlag) {
                        ...
                        const ret = await (0, schedule_template_1.runSchedule)(t, this.month, budgeted, remainder, this.fromLastMonth, toBudget, [], this.category);
    ...
    //-----------------------------------------------------------------------------
    // Implementation
    category; //readonly so we can double check the category this is using
    month;
    templates = [];
    remainder = [];
    goals = [];
    ...
    fullAmount = null; // the full requested amount, start null for remainder only cats
    isLongGoal = null; //defaulting the goals to null so templates can be unset
    goalAmount = null;
    ...
    static async checkByAndScheduleAndSpend(templates, month) {
        if (templates.filter(t => t.type === 'schedule' || t.type === 'by').length ===
            0) {
            return;
        }
        //check schedule names
        const scheduleNames = (await (0, statements_1.getActiveSchedules)()).map(({ name }) => name.trim());
        templates
        ...
        const lowestPriority = Math.min(...templates
        ...
        templates
            ...
            .forEach(t => {
            const range = monthUtils.differenceInCalendarMonths(`${t.month}`, month);
    ...
    static async checkPercentage(templates) {
        ...
        reqCategories.forEach(n => {
            if (n === 'available funds' || n === 'all income') {
                ...
                throw new Error(`Category \x22${n}\x22 is not found in available income categories`);
            }
        });
    }
    checkLimit(templates) {
        for (const template of templates.filter(t => t.type === 'simple' ||
            t.type === 'periodic' ||
            t.type === 'limit' ||
            t.type === 'remainder')) {
            let limitDef;
            ...
            if (limitDef.period === 'daily') {
                const numDays = monthUtils.differenceInCalendarDays(monthUtils.addMonths(this.month, 1), this.month);
                ...
                const baseLimit = (0, util_1.amountToInteger)(limitDef.amount);
    ...
    static async runCopy(template, templateContext) {
        const sheetName = monthUtils.sheetForMonth(monthUtils.subMonths(templateContext.month, template.lookBack));
        return await (0, actions_1.getSheetValue)(sheetName, `budget-${templateContext.category.id}`);
    }
    static runPeriodic(template, templateContext) {
        let toBudget = 0;
        const amount = (0, util_1.amountToInteger)(template.amount);
        ...
        switch (period) {
            ...
            case 'year':
                ...
                dateShiftFunction = (date, numPeriods) => monthUtils.addMonths(date, numPeriods * 12);
                break;
        }
    ...
    static async runSpend(template, templateContext) {
        ...
        const repeat = template.annual
            ...
            : template.repeat;
        let m = monthUtils.differenceInCalendarMonths(toMonth, templateContext.month);
        if (repeat && m < 0) {
            while (m < 0) {
        ...
        for (let m = fromMonth; monthUtils.differenceInCalendarMonths(templateContext.month, m) > 0; m = monthUtils.addMonths(m, 1)) {
            const sheetName = monthUtils.sheetForMonth(m);
            if (firstMonth) {
                //TODO figure out if I already  found these values and can pass them in
                const spent = await (0, actions_1.getSheetValue)(sheetName, `sum-amount-${templateContext.category.id}`);
                const balance = await (0, actions_1.getSheetValue)(sheetName, `leftover-${templateContext.category.id}`);
                alreadyBudgeted = balance - spent;
                firstMonth = false;
            }
            else {
                alreadyBudgeted += await (0, actions_1.getSheetValue)(sheetName, `budget-${templateContext.category.id}`);
            }
        }
        const numMonths = monthUtils.differenceInCalendarMonths(toMonth, templateContext.month);
        const target = (0, util_1.amountToInteger)(template.amount);
        if (numMonths < 0) {
            return 0;
        }
        else {
    ...
    static async runPercentage(template, availableFunds, templateContext) {
        ...
        if (cat === 'all income') {
            monthlyIncome = await (0, actions_1.getSheetValue)(sheetName, `total-income`);
        }
            ...
            monthlyIncome = await (0, actions_1.getSheetValue)(sheetName, `sum-amount-${incomeCat.id}`);
        }
    ...
    static async runAverage(template, templateContext) {
        ...
        for (let i = 1; i <= template.numMonths; i++) {
            const sheetName = monthUtils.sheetForMonth(monthUtils.subMonths(templateContext.month, i));
            sum += await (0, actions_1.getSheetValue)(sheetName, `sum-amount-${templateContext.category.id}`);
        }
        return -Math.round(sum / template.numMonths);
    }
    static runBy(templateContext) {
        const byTemplates = templateContext.templates.filter(t => t.type === 'by');
        const savedInfo = [];
        let totalNeeded = 0;
        let shortNumMonths;
        ...
        for (let i = 0; i < byTemplates.length; i++) {
            ...
            const period = template.annual
                ...
                : template.repeat != null
                    ? template.repeat
                    : null;
            let numMonths = monthUtils.differenceInCalendarMonths(targetMonth, templateContext.month);
...
//# sourceMappingURL=category-template-context.js.map