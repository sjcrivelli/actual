...
const cleanup_template_pegjs_1 = require("./cleanup-template.pegjs");
    ...
    while (groupLength > 0) {
        ...
        if (sinkGroup.length > 0 || generalGroup.length > 0) {
            ...
            for (let ii = 0; ii < tempSourceGroups.length; ii++) {
                const balance = await (0, actions_1.getSheetValue)(sheetName, `leftover-${tempSourceGroups[ii].category}`);
                const budgeted = await (0, actions_1.getSheetValue)(sheetName, `budget-${tempSourceGroups[ii].category}`);
                await (0, actions_1.setBudget)({
            ...
            for (let ii = 0; ii < generalGroup.length && available_amount > 0; ii++) {
                const balance = await (0, actions_1.getSheetValue)(sheetName, `leftover-${generalGroup[ii].category}`);
                const budgeted = await (0, actions_1.getSheetValue)(sheetName, `budget-${generalGroup[ii].category}`);
                const to_budget = budgeted + Math.abs(balance);
                const categoryId = generalGroup[ii].category;
                let carryover = await db.first(`SELECT carryover FROM zero_budgets WHERE month = ? and category = ?`, [db_month, categoryId]);
                if (carryover === null) {
            ...
            for (let ii = 0; ii < sinkGroup.length && available_amount > 0; ii++) {
                const budgeted = await (0, actions_1.getSheetValue)(sheetName, `budget-${sinkGroup[ii].category}`);
                const to_budget = budgeted +
...
async function processCleanup(month) {
    ...
    for (let c = 0; c < categories.length; c++) {
        ...
        if (template) {
            if (template.filter(t => t.type === 'source' && t.group === null).length > 0) {
                const balance = await (0, actions_1.getSheetValue)(sheetName, `leftover-${category.id}`);
                const budgeted = await (0, actions_1.getSheetValue)(sheetName, `budget-${category.id}`);
                if (balance >= 0) {
                ...
                const carryover = await db.first(`SELECT carryover FROM zero_budgets WHERE month = ? and category = ?`, [db_month, category.id]);
                if (carryover !== null) {
    ...
    for (let c = 0; c < categories.length; c++) {
        const category = categories[c];
        const budgetAvailable = await (0, actions_1.getSheetValue)(sheetName, `to-budget`);
        const balance = await (0, actions_1.getSheetValue)(sheetName, `leftover-${category.id}`);
        const budgeted = await (0, actions_1.getSheetValue)(sheetName, `budget-${category.id}`);
        const to_budget = budgeted + Math.abs(balance);
    ...
    for (let c = 0; c < sinkCategory.length; c++) {
        const budgeted = await (0, actions_1.getSheetValue)(sheetName, `budget-${sinkCategory[c].cat.id}`);
        ...
        if (c === sinkCategory.length - 1) {
            const currentBudgetAvailable = await (0, actions_1.getSheetValue)(sheetName, `to-budget`);
...
async function getCategoryTemplates() {
    const templates = {};
    const notes = await db.all(`SELECT * FROM notes WHERE lower(note) like '%${TEMPLATE_PREFIX}%'`);
    for (let n = 0; n < notes.length; n++) {
        ...
        for (let l = 0; l < lines.length; l++) {
            ...
            try {
                const parsed = (0, cleanup_template_pegjs_1.parse)(expression);
...
//# sourceMappingURL=cleanup-template.js.map