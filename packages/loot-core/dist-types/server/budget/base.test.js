...
describe('Base budget', () => {
    it('Recomputes budget cells when account fields change', async () => {
        ...
        await (0, base_1.createAllBudgets)();
        ...
        expect(sheet.getCellValue(monthUtils.sheetForMonth('2016-12'), `sum-amount-${catId}`)).toBe(0);
        ...
        expect(sheet.getCellValue(monthUtils.sheetForMonth('2016-12'), `sum-amount-${catId}`)).toBe(-5000);
    });
    it('Excludes hidden categories from group totals in Report Budget', async () => {
        ...
        expect(sheet.getCellValue(sheetName, `sum-amount-${visibleCatId}`)).toBe(-1000);
        expect(sheet.getCellValue(sheetName, `sum-amount-${hiddenCatId}`)).toBe(-2000);
    ...
    it('Excludes hidden category groups from budget totals in Report Budget', async () => {
        ...
        expect(sheet.getCellValue(sheetName, `sum-amount-${visibleGroupCatId}`)).toBe(-1000);
        expect(sheet.getCellValue(sheetName, `sum-amount-${hiddenGroupCatId}`)).toBe(-2000);
