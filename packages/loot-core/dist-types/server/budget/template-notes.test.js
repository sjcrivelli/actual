...
describe('storeNoteTemplates', () => {
    ...
    it.each(testCases)('$description', async ({ mockTemplateNotes, expectedTemplates }) => {
        // Given
...
describe('checkTemplates', () => {
    ...
    it.each(testCases)('$description', async ({ mockTemplateNotes, mockSchedules, expected }) => {
        // Given
        mockGetTemplateNotesForCategories(mockTemplateNotes);
        mockGetActiveSchedules(mockSchedules);
        // When
        const result = await (0, template_notes_1.checkTemplateNotes)();
        // Then
...
describe('unparse/parse round-trip', () => {
    ...
    it.each(cases)('round-trips: %s', async (original) => {
        const parsed = (0, goal_template_pegjs_1.parse)(original);
        const serialized = await (0, template_notes_1.unparse)([parsed]);
        const reparsed = (0, goal_template_pegjs_1.parse)(serialized);
        expect(parsed).toEqual(reparsed);
    });
});
//# sourceMappingURL=template-notes.test.js.map