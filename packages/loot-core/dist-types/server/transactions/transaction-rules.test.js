...
async function getMatchingTransactions(conds) {
    const { filters } = (0, transaction_rules_1.conditionsToAQL)(conds);
...
describe('Transaction rules', () => {
    ...
    test('insert a rule into the database', async () => {
        ...
        await (0, transaction_rules_1.insertRule)({
            stage: 'pre',
            conditionsOp: 'and',
            conditions: [],
            actions: [],
        ...
        const transaction = await (0, transaction_rules_1.runRules)({
            date: '2019-05-10',
            notes: '',
            category: null,
        });
    ...
    test('update a rule in the database', async () => {
        await (0, transaction_rules_1.loadRules)();
        const id = await (0, transaction_rules_1.insertRule)({
            stage: 'pre',
            conditionsOp: 'and',
        ...
        expect((0, transaction_rules_1.getRules)().length).toBe(1);
        let transaction = await (0, transaction_rules_1.runRules)({
        ...
        expect((0, transaction_rules_1.getRules)().length).toBe(1);
        transaction = await (0, transaction_rules_1.runRules)({
            imported_payee: 'Kroger',
            notes: '',
            category: null,
        });
    ...
    test('delete a rule in the database', async () => {
        ...
        expect(transaction.category).toBe('food');
        await (0, transaction_rules_1.deleteRule)(id);
    ...
    test('ids in rules are migrated as mapping changes', async () => {
        ...
        let rule1 = (0, transaction_rules_1.getRules)().find(r => r.id === 'one');
        let rule2 = (0, transaction_rules_1.getRules)().find(r => r.id === 'two');
        ...
        rule1 = (0, transaction_rules_1.getRules)().find(r => r.id === 'one');
        rule2 = (0, transaction_rules_1.getRules)().find(r => r.id === 'two');
        expect(rule1.actions[0].value).toBe('home_id');
        expect(rule2.conditions[0].value).toBe('home_id');
        expect(rule2.conditions[1].value).toBe('beer_id');
    });
    test('await runRules runs all the rules in each phase', async () => {
        ...
        expect(await (0, transaction_rules_1.runRules)({
...
describe('Learning categories', () => {
    ...
    async function insertTransaction(transaction, expectedCategory, expectedRuleCount = 1, expectedPayee = 'foo') {
        await db.insertTransaction(transaction);
        await (0, transaction_rules_1.updateCategoryRules)([transaction]);
    ...
    test('getProbableCategory estimates a category winner', () => {
        let winner = (0, transaction_rules_1.getProbableCategory)([{ category: 'foo' }]);
    ...
    test('updates rules correctly even if multiple rules exist', async () => {
        ...
        await (0, transaction_rules_1.updateCategoryRules)([{ ...trans, id: 'three' }]);
        ...
        await (0, transaction_rules_1.updateCategoryRules)([{ ...trans, id: 'three' }]);
        expect((0, transaction_rules_1.getRules)()).toMatchSnapshot();
        const rules = (0, transaction_rules_1.getRules)();
        const getPayees = cat => {
            const arr = rules
    ...
    test('rules are saved with internal field names', async () => {
        ...
        let [rule] = (0, transaction_rules_1.getRules)();
        ...
        // public names
        [rule] = (0, transaction_rules_1.getRules)();
        expect(rule.conditions[0].field).toBe('imported_payee');
        expect(rule.actions[0].field).toBe('payee');
    });
    test('rules with public field names are loaded correctly', async () => {
        ...
        const [rule] = (0, transaction_rules_1.getRules)();
        expect(rule.conditions[0].field).toBe('imported_payee');
        expect(rule.actions[0].field).toBe('payee');
    });
    // TODO: write tests for split transactions
});
//# sourceMappingURL=transaction-rules.test.js.map