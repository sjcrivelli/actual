...
async function execTransactionsGrouped(compilerState, queryState, sqlPieces, params, outputTypes) {
    ...
    if ((0, compiler_1.isAggregateQuery)(queryState)) {
        ...
        return (0, exec_1.execQuery)(queryState, compilerState, s, params, outputTypes);
    }
    let rows;
    let matched = null;
    ...
    const where = (0, util_1.whereIn)(rows.map(row => row.group_id), `IFNULL(${sqlPieces.from}.parent_id, ${sqlPieces.from}.id)`);
    const finalSql = `
...
async function execCategoryGroupsWithCategories(compilerState, queryState, sqlPieces, params, categoriesOption, outputTypes) {
    ...
    const { data: categories } = await (0, __1.aqlQuery)((0, query_1.q)('categories')
        .filter({
...
async function execCategoryGroupsBasic(compilerState, queryState, sqlPieces, params, outputTypes) {
    return (0, exec_1.execQuery)(queryState, compilerState, sqlPieces, params, outputTypes);
}
