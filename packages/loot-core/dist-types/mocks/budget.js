...
const sync_1 = require("../server/accounts/sync");
const aql_1 = require("../server/aql");
const budgetActions = __importStar(require("../server/budget/actions"));
const budget = __importStar(require("../server/budget/base"));
...
const sync_2 = require("../server/sync");
...
async function fillPrimaryChecking(handlers, account, payees, groups) {
    ...
    for (let i = 0; i < numTransactions; i++) {
        ...
        const currentDate = monthUtils.subDays(monthUtils.currentDay(), Math.floor(i / 3));
        const transaction = {
            id: (0, uuid_1.v4)(),
            amount,
            payee: payee.id,
    ...
    const earliestMonth = monthUtils.monthFromDate(transactions[transactions.length - 1].date);
    const months = monthUtils.rangeInclusive(earliestMonth, monthUtils.currentMonth());
    ...
    return (0, sync_1.addTransactions)(account.id, transactions);
}
...
async function fillSavings(handlers, account, payees, groups) {
    ...
    for (let i = 0; i < numTransactions; i++) {
        ...
        const category = payee === incomePayee
...
async function createBudget(accounts, payees, groups) {
    ...
    const earliestDate = (await db.first(`SELECT t.date FROM v_transactions t LEFT JOIN accounts a ON t.account = a.id
       WHERE a.offbudget = 0 AND t.is_child = 0 ORDER BY date ASC LIMIT 1`)).date;
    const earliestPrimaryDate = (await db.first(`SELECT t.date FROM v_transactions t LEFT JOIN accounts a ON t.account = a.id
    ...
    function setBudgetIfSpent(month, cat) {
        const spent = sheet.getCellValue(monthUtils.sheetForMonth(month), `sum-amount-${cat.id}`);
        if (spent < 0) {
            setBudget(month, cat, -spent);
        }
    }
    await (0, mutators_1.runMutator)(() => (0, sync_2.batchMessages)(async () => {
        for (const month of months) {
            if (month >=
    ...
    const toBudget = sheet.getCellValue(sheetName, 'to-budget');
    if (toBudget < 0) {
        await (0, sync_1.addTransactions)(primaryAccount.id, [
            {
                amount: -toBudget,
...
async function createTestBudget(handlers) {
    ...
    const allGroups = (await (0, mutators_1.runHandler)(handlers['get-categories'])).grouped;
    (0, sync_2.setSyncingMode)('import');
    await (0, mutators_1.runMutator)(() => (0, sync_2.batchMessages)(async () => {
        for (const account of accounts) {
            ...
            else if (account.name === 'Capital One Checking' ||
                account.name === 'HSBC') {
            ...
            else if (account.name === 'Vanguard 401k' ||
    ...
    const primaryAccount = accounts.find(a => (a.name = 'Bank of America'));
    const { data: primaryBalance } = await (0, aql_1.aqlQuery)((0, query_1.q)('transactions')
        .filter({ account: primaryAccount.id })
        .calculate({ $sum: '$amount' })
        .serialize());
    if (primaryBalance < 0) {
        const { data: results } = await (0, aql_1.aqlQuery)((0, query_1.q)('transactions')
            .filter({ account: primaryAccount.id, amount: { $gt: 0 } })
            .limit(1)
            .select(['id', 'amount'])
            .serialize());
        const lastDeposit = results[0];
        await (0, mutators_1.runHandler)(handlers['transaction-update'], {
            ...lastDeposit,
    ...
    await (0, mutators_1.runMutator)(() => (0, sync_2.batchMessages)(async () => {
        ...
        await (0, mutators_1.runHandler)(handlers['schedule/create'], {
            schedule: {
                name: 'Phone bills',
